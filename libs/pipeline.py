...
async def _pipeline_publish_yaml(
    build: BuildResult,
    review: ReviewResult,
    infer: InferResult,
    *,
    ...
    """Legacy publish for YAML packages using convert_to_storage from cli.lang_to_storage.

    Args:
        build: Result from pipeline_build (must have graph_data with 'package' key).
        review: Result from _pipeline_review_yaml (ReviewResult with merged_package).
        infer: Result from pipeline_infer.
        storage_manager: Pre-initialized StorageManager (server/batch use — not closed after).
        storage_config: StorageConfig to create a new manager (CLI use — closed after).
        db_path: LanceDB path shortcut (used if storage_config is None).
