...
> 当前 v0.5 CLI 已实现 Python package、`gaia build compile`、`gaia pkg add`、
> `gaia run infer`、`gaia pkg register` 的 source-registry contract。本文中的
> Review Server assignment、`.gaia/reviews/` 入库 gate、validated review
> reports、LKM global snapshot 是生态层设计目标，不是当前 Gaia CLI 已强制
> 执行的注册流程。
...
**② 编译 + 本地推理预览。** `gaia build compile` 将源码确定性地编译为结构化推理图。`gaia run infer` 在本地运行推理，让 Alice 在发布前预览可信度——如果结论可信度很低，可能需要补充论证。
→ 详见 [03 包的创建与发布](03-authoring-and-publishing.md)
...
- [Gaia IR design](../gaia-ir/01-overview.md) — 当前 Python IR 模型与编译输出接口
