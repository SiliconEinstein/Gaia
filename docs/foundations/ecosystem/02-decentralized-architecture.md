...
> 当前 v0.5 CLI 已实现 Python package、`gaia compile`、`gaia add`、
> `gaia infer`、`gaia register` 的 source-registry contract。本文中的
...
**② 编译 + 本地推理预览。** `gaia compile` 将源码确定性地编译为结构化推理图。`gaia infer` 在本地运行推理，让 Alice 在发布前预览可信度——如果结论可信度很低，可能需要补充论证。
→ 详见 [03 包的创建与发布](03-authoring-and-publishing.md)
