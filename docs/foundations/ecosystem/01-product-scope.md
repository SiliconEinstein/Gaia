...
- **目标本地管线**是 `build -> self-review skill -> graph-construction skill -> infer -> publish`，在 v0.5 alpha-0 的 grouped CLI 上对应 `gaia build init/compile/check` → `gaia inquiry review / focus / hypothesis` → `gaia run infer` → `gaia run render` → `gaia pkg register`；详见 [`../cli/workflow.md`](../cli/workflow.md) 与 [`../../releases/migration-alpha-0.md`](../../releases/migration-alpha-0.md)
