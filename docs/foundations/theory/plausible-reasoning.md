...
四个三段论都建立在同一个蕴含关系 A → B 上。在因子图的语境下（参见 [reasoning-hypergraph.md](reasoning-hypergraph.md)），它们不是四种不同的算子，而是同一个 **entailment 因子**上信念传播的四种消息方向：

| 三段论 | 消息方向 | 效果 |
|--------|---------|------|
| 强三段论 (modus ponens) | 正向：A → B | A 的高信念推高 B |
| 弱三段论 3 (弱否认) | 正向：A → B | A 的低信念略微压低 B |
| 弱三段论 1 (弱确认) | 反向：B → A | B 的高信念提升 A |
| 弱三段论 2 (modus tollens) | 反向：B → A | B 的低信念压低 A |

这意味着 **entailment 是最基本的推理算子**——一旦一个 entailment 因子被建立，双向消息传递自动实现了科学推理中的全部确认和反驳逻辑。
...
```
induction/abduction  →  发现候选 entailment  →  验证  →  永久 entailment
     (探索)                  (提议)            (确认)       (利用)
```

一旦 entailment 被确认为永久关系，Polya/Jaynes 的四个三段论就自动生效——既能正向推导结论，也能反向确认或反驳前提。这是概率推理框架的核心力量：**探索性推理（归纳、溯因）的终极目标是建立确定性推理（蕴含），而确定性推理一旦建立就自动获得了全部四种三段论的推理能力。**

#### Analogy 不是独立算子

§1.2 中列出的类比（analogy）不需要独立的算子类型——它可以分解为已有算子的组合：

```
领域 A: "声波有衍射"  ─── induction ───> Template: "{波}有衍射"
                                              │
                                         instantiation
                                              │
领域 B: "光表现得像波" ──────────────────> "光可能有衍射"
...
类比的本质是：从领域 A 归纳出模板（induction），再将模板实例化到领域 B（instantiation）。或者等价地，观测到 B 与 A 的结构相似性后，溯因提议同一个 entailment 也适用于 B（abduction）。

无论哪种分解，类比都不引入新的逻辑原语——它是 induction、instantiation 和 abduction 的复合操作。
