...
> **关于因果机制的辖区说明**：本文档定义的"信息 `I`" 和"约束集 `C(I)`"涵盖**命题层**的概率约束和逻辑约束（§3.1、§3.2）。当 `I` 还包含 `Mechanism` 这类结构性知识（v0.6+）时，机制声明**不进入 `C(I)`**——它们没有真值、不能写成 `E[f(X)] = c` 这种命题约束的形式，而是直接 lower 成 `CausalFactor` 进入因子图。命题层的所有 worked example（§6、§7）和 MaxEnt 优化目标在含 `Mechanism` 的图上**完全不变**，机制只是在乘积分解中贡献一个 fully-specified 的局部因子，不参与 MaxEnt 自由度的选择。详见 [`08-causality-and-jaynes.md`](08-causality-and-jaynes.md) §5。
