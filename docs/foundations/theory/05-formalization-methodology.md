...
↝（软蕴含）算子文档（参见 [03-propositional-operators.md](03-propositional-operators.md)）指出，整个推理系统中唯一的自由参数是作者给出的**条件概率对 `(p₁, p₂)`** — "这条推理在正向支持和前提缺席时各有多强"。其他一切要么由逻辑真值表唯一确定（确定性派生算子 →、↔、⊗、⊕ 及基础运算 ¬、∧），要么随网络证据积累而递减（节点先验 π），要么由知识内容本身决定（网络结构）。
