...
### 2.1 Step 1 — 提取 claim 与粗推理链

从科学家的自然语言论述（论文、专著、实验报告）中提取：

- **Claim（变量节点）**：每个可以为真或假的断言。**每个 claim 必须 self-contained** — 不依赖其他 claim 的上下文，单独阅读就能理解。
- **推理链（因子节点）**：claim 之间的推理关系。这一步先标记为 `infer`（未分类），不急于确定算子类型。

这一步产生的图是粗糙的：推理链的算子类型和布线方向尚不明确。

### 2.2 Step 2 — 识别弱点

粗图中的推理链分为两类：

- **Entailment（蕴含）**：前提真则结论必然为真，p ≈ 1。这不是弱点。
- **非 entailment**：abduction（溯因）、induction（归纳）、analogy（类比）。这些步骤的 p 不确定，是**弱点**。

弱点的本质是：推理中存在逻辑蕴含之外的跳跃 — 从观测到假说、从实例到规律、从已知域到未知域。Step 3 的目标是将这些跳跃分解为更基本的结构。

### 2.3 Step 3 — 用子图模式分解弱点

每种弱点对应一个**可复用的子图模式**，由 entailment + observation + equivalence 组合而成。分解后所有链的 p ≈ 1，不确定性从"推理链的强度"转移到"命题是否为真"。
...
### 3.2 Step 1 — 提取 claim 与粗推理链

从原文中提取 self-contained 的 claim：

| 节点 | 内容（self-contained） |
|------|----------------------|
| **A** | 物体下落速度与其重量成正比：重物比轻物下落更快 |
| **O_daily** | 日常生活中，石头比羽毛下落更快，铁球比木球更快 |
| **O_media** | 在不同介质中比较轻重物体的下落，介质越稠密，速度差异越明显；越稀薄，差异越不明显 |
| **O_air** | 在空气中，金球、铅球、铜球等重材料球从 100 腕尺落下，金球领先铜球不超过四指 |
| **H** | 不同重量物体的下落速度差异由介质阻力造成，而非由重量本身决定 |
| **V** | 在真空（无阻力）环境中，不同重量的物体以相同速率下落 |
| **T₁** | 假设重物下落更快，将重球 H 与轻球 L 绑在一起，L 拖拽 H，复合体 HL 的速度应慢于 H 单独下落 |
| **T₂** | 假设重物下落更快，复合体 HL 的总重量大于 H，因此 HL 的速度应快于 H 单独下落 |
| **E_θᵢ** | 在斜面倾角 θᵢ 下，不同重量的光滑球沿抛光沟槽滚下，所经距离之比等于时间之比的平方，与球的重量无关 |

粗推理链（算子待定）：

```
O_daily --[infer]--> A                         "日常观测→重者更快"
A --[entailment, p≈1]--> T₁                    "若重者快→拖拽论证"
A --[entailment, p≈1]--> T₂                    "若重者快→总重论证"
T₁ + T₂ --[contradiction]-->                    "同一前提推出矛盾"
O_media + O_air --[infer]--> H                  "介质观测→空气阻力假说"
H --[infer]--> V                                "假说→真空预测"
E_θᵢ --[infer]--> V                             "斜面实验→等速下落"
A + V --[contradiction]-->                       "重者更快 vs 等速下落"
```

entailment 和 contradiction 已经确定。标为 `infer` 的四条链是弱点。

### 3.3 Step 2 — 识别弱点

| 弱点 | 原始链 | 推理类型 |
|------|-------|---------|
| **W1** | O_daily → A | induction：从日常观测归纳出一般规律 |
| **W2** | O_media + O_air → H | abduction：从观测推断最佳解释 |
| **W3** | H → V | analogy：从有阻力介质的模式外推到真空极限 |
| **W4** | E_θᵢ → V | induction：从斜面实验归纳出等速下落 |

### 3.4 Step 3 — 分解弱点

#### W1：日常观测 → 重者更快（Induction 模式）
...
#### W3：空气阻力假说 → 真空等速（Analogy 模式）

**Part a — Induction 建立密度-差异递减模式 G：**
...
2. **观测路径**：W2 子图提升 H → W3 子图将 H + G + C_cont + S_vacuum 蕴含 V
3. **实验路径**：W4 子图中大量实验观测通过反向消息直接支撑 V
...
- A 被连球悖论压低（逻辑路径），同时 W1 的两个日常观测仅提供有限支撑 → A 的 belief 趋向 ε
- H 被 W2 的介质观测支撑 → belief 高
...
| 形式化步骤 | 科学实践 |
|-----------|---------|
| 提取 self-contained claim | 要求论文中每个论断清晰、独立、可验证 |
| 识别 abduction/induction/analogy 弱点 | 同行评审质疑论证中的薄弱环节 |
| 分解为 entailment + equivalence | 要求作者给出假说的可测预测，并与实际观测对照 |
| 显式化隐含前提（如连续性假设） | 审稿人要求作者明确假设和适用条件 |
