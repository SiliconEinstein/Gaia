...
> - [reasoning-factor-graph.md](reasoning-factor-graph.md) — 因子图结构与四种逻辑原语算子
> - [coarse-reasoning.md](coarse-reasoning.md) — 粗推理算子、粗/细因子图、细化工作流
> - [science-ontology.md](science-ontology.md) — 知识类型、关系类型、七种推理策略
> - [belief-propagation.md](belief-propagation.md) — 因子图上的置信传播（BP）算法

## 1. 问题：p 是唯一的主观参数

粗推理算子文档（参见 [coarse-reasoning.md](coarse-reasoning.md)）指出，整个推理系统中唯一的因子级自由参数是作者给出的**条件概率 p** — "这条推理的形式化完整程度"。其他一切要么由逻辑真值表唯一确定（四种逻辑原语算子），要么随网络证据积累而递减（节点先验 π），要么由知识内容本身决定（图结构）。
...
以下三种子图模式对应 [science-ontology.md](science-ontology.md) §3 中定义的推理策略的细因子图展开。

#### Abduction 模式（溯因）

认识论上，溯因是"观测到 B'，推断最佳解释 A"（参见 [science-ontology.md](science-ontology.md) §3.2）。在因子图中分解为：
...
归纳是"从多个实例推断一般规律"（参见 [science-ontology.md](science-ontology.md) §3.3）。本质上是**重复的 abduction** — 一般规律是"假说"，每个实例是一个"预测+观测"对：
...
类比是把源域中已经建立的结构规律迁移到目标域（参见 [science-ontology.md](science-ontology.md) §3.4）。其关键不是连续性，而是一个显式的桥梁主张 `M`：
...
Step 2 产生的粗因子图已经具有推理价值——它捕捉了推理骨架，标注了哪些步骤已充分形式化（entailment，p ≈ 1）、哪些尚未充分形式化（weakpoint，p < 1）。粗因子图上可以直接运行 BP，产生有意义的 belief 值（参见 [coarse-reasoning.md](coarse-reasoning.md) §6）。
...
当所有链的 p ≈ 1 后，系统中剩余的自由度只有原子命题的先验 π。Cox 定理保证：给定网络结构和 p 值，存在唯一的一致性 belief 赋值。当 equivalence 和 contradiction 足够多时，π 的初始选择变得无关紧要（参见 [belief-propagation.md](belief-propagation.md) §3）。
...
关于粗因子图与细因子图的形式定义及细化的局部性质，参见 [coarse-reasoning.md](coarse-reasoning.md) §6。
...
#### W₁：日常观测 → A（Induction 模式，参见 [science-ontology.md](science-ontology.md) §3.3）
...
#### W₂：介质观测 → V（Abduction + Entailment，参见 [science-ontology.md](science-ontology.md) §3.2）
...
#### W₃：斜面实验 → V（Induction 模式，参见 [science-ontology.md](science-ontology.md) §3.3）
...
那么 π 的影响被因子消息淹没（参见 [belief-propagation.md](belief-propagation.md) §3），belief 收敛到由网络拓扑唯一决定的值。**充分的形式化消除了参数选择的任意性，使得 belief 成为知识结构的客观函数。**
