...
第一阶量化结构在 v0.5 已经有具体编程语言实现：authoring 层的 `gaia.engine.lang.formula`（`Forall` / `Exists` / `Predicate` / `Term`）以 AST 形式表达 Template，propositional 化简则由 `gaia.engine.ir.logic` 完成；详见 [`../gaia-lang/predicate-logic.md`](../gaia-lang/predicate-logic.md)。
