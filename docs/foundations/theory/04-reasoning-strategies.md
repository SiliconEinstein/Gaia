...
>
> 本文档定义知识类型和推理策略。每种策略是 ↝（似然蕴含）的微观结构还原。
...
在 `03-propositional-operators.md` 中，我们定义了似然蕴含 ↝ 并指出：
...
3. **Bayes 推导（derivation）：** 从微观结构出发，用 Bayes 定律推导出宏观 ↝ 的有效参数 (p₁, p₂)
...
- **上游：**
  - [03-propositional-operators.md](03-propositional-operators.md) — 最小原料 {¬, ∧, π}、派生算子、关系类型、似然蕴含 ↝ 的定义与理论地位
