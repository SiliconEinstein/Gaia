...
当 M 有高信念时（q 小），msg(f→C) ≈ [p₁, 1−p₁]。p₁ 越大，C=1 越被偏好——这就是软蕴含的正向支持。当 M 信念低时（q 大），msg(f→C) ≈ [1−p₂, p₂]。当 p₂ = 0.5 时消息接近均匀——前提为假时不提供信息（MaxEnt 无信息值）；当 p₂ > 0.5 时 C=0 被偏好——前提缺席使结论倾向为假。
...
- 当 C 信念低时（r 大），msg(f→M) ≈ [1−p₁, p₂]。由于 ↝ 满足 `p₁ + p₂ > 1`，此时 M=1 的权重低于 M=0——modus tollens：结论为假削弱前提。
...
**弱三段论 2（结论假 → 前提信念下降）：** §2.2 的反向消息分析表明，当 C 信念低时（r 大），msg(f→M) ≈ [1−p₁, p₂]。由于 `p₁ + p₂ > 1`，M=1 获得的权重低于 M=0——modus tollens：结论为假削弱前提。
