...
| 术语 | Gaia 中的对应 | 代码位置 |
|------|-------------|---------|
| Variable (变量) | Knowledge node | `libs/storage/models.py:Knowledge` |
| Prior (先验) | Knowledge.prior | `libs/storage/models.py` |
| Belief (后验信念) | BeliefSnapshot.belief | `libs/storage/models.py` |
| Factor (因子) | FactorNode | `libs/storage/models.py:FactorNode` |
| Factor potential (势函数) | `_evaluate_potential()` | `libs/inference/bp.py` |
...
| Gate (已移除) | 旧设计中关系节点的只读机制，v2.0 已统一为普通参与者 | — |
