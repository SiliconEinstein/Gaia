...
> 工程实现细节（IR schema、`mutilate()` 算法、do-calculus identification）参见对应 spec：
> - [`docs/specs/2026-05-06-causal-mechanism-first-class-design.md`](../../specs/2026-05-06-causal-mechanism-first-class-design.md)
> - [`docs/specs/2026-05-06-causal-counterfactual-binary-noise-design.md`](../../specs/2026-05-06-causal-counterfactual-binary-noise-design.md)
> - [`docs/specs/2026-05-06-causal-transport-y0-design.md`](../../specs/2026-05-06-causal-transport-y0-design.md)
...
为容纳因果机制，Gaia 在 v0.6 把知识从 v0.5 的"全部都是命题"扩展到**三层本体**（参见 [mechanism-first-class spec §0](../../specs/2026-05-06-causal-mechanism-first-class-design.md)）：
...
[mechanism-first-class spec §0](../../specs/2026-05-06-causal-mechanism-first-class-design.md) 给出了五条工程症状，解释为什么把因果边塞进 `Claim(kind=CAUSAL)` 不通。这里只摘最 fundamental 的一条。
...
- 当 `I` 只包含命题约束时，v0.5 的 worked examples（[02 §6 Horn 规则](02-maxent-grounding.md#6-worked-example-1只有硬-horn-规则)、[02 §7 软约束](02-maxent-grounding.md#7-worked-example-2软约束版本的-ab---c-与-d---c)）**完全有效，无需任何修正**；
...
**关键设计**（参见 [mechanism-first-class spec §6.3](../../specs/2026-05-06-causal-mechanism-first-class-design.md) 与 [§4.4](../../specs/2026-05-06-causal-mechanism-first-class-design.md)）：
...
实际效果：**MaxEnt 自由度只在命题层声明，结构层（`Mechanism`）的参数由作者直接给（`cpd=(α, β)`），没有 MaxEnt 选择**。这与 v0.5 [02 §3.3 局部特征就够了](02-maxent-grounding.md#33-为什么局部特征就够了) 描述的"乘积分解 = MaxEnt + 硬约束的自然结果"在数学上自洽——`CausalFactor` 是 fully-specified 的局部因子，它给乘积分解贡献一个完整的局部项，不参与 MaxEnt 优化。
...
1. 复制因子图 `FG → FG'`；
2. 在 `FG'` 中删除所有 `CausalFactor` 满足"其 conclusion ∈ {X}"——也就是切掉 X 的因果入边（详见 [mechanism-first-class spec §6.3](../../specs/2026-05-06-causal-mechanism-first-class-design.md)）；
...
[counterfactual-binary-noise spec](../../specs/2026-05-06-causal-counterfactual-binary-noise-design.md) 把 Pearl level 3 的反事实查询纳入 v0.6 范围。本文档只讲它在本体框架下的位置，工程细节不重复。
...
[mechanism-first-class spec §4.4 思想实验 1–8](../../specs/2026-05-06-causal-mechanism-first-class-design.md) 给出了 8 个具体例子来训练这个直觉。
...
> 与 [mechanism-first-class spec §1.2](../../specs/2026-05-06-causal-mechanism-first-class-design.md) 重复声明——避免读者从理论文档进入时困惑"为什么不直接用 DoWhy"。
