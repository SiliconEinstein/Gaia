...
6. **结构性知识**：除了概率约束（"P(C) = 0.7"）和逻辑约束（"A → B"），科学推理还涉及结构性声明（"X 是 Y 的因"、"机制是 M"）。这类知识没有真值，不参与命题层的 belief update，但决定哪些干预性 / 反事实查询有意义。详见 [`08-causality-and-jaynes.md`](08-causality-and-jaynes.md)。
...
- 一个能区分命题与世界结构的本体（详见 [`08-causality-and-jaynes.md`](08-causality-and-jaynes.md)）
