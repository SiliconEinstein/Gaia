...
> - **因子图中的逻辑约束**（如 03-propositional-operators.md 中定义的硬蕴含、互斥约束）使用精确的 0/1 势函数，这完全符合概率论。
...
这些概率关系在大规模科学知识上形成一个需要计算结构和算法的网络。关于如何从约束、MaxEnt 和相对熵更新得到具体 posterior，参见 [02-maxent-grounding.md](02-maxent-grounding.md)。关于推理超图（因子图）的数学结构和 Belief Propagation 算法，参见 [07-belief-propagation.md](07-belief-propagation.md)。
