...
2. **合并重复命题** — Curation Server 发现两个命题是同一个，合并后需要重算
3. **矛盾确认** — 两个命题被确认互相矛盾
...
```
Curation Server 发现命题 P 和命题 Q 矛盾
  ↓
...
| **Review Server** | 不涉及 | 审核触发增量推理 | 不直接触发 |
| **Curation Server** | 不涉及 | 合并/矛盾触发增量推理 | 跨 Registry 关系触发全局推理 |
