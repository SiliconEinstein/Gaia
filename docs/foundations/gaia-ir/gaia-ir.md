...
| **`abduction`** | 单参数 `[p]`（折叠时） | Strategy 或 FormalStrategy | 可先粗表示为叶子；prediction / observation 节点显式化后可直接 FormalStrategy |
| **`induction`** | 单参数 `[p]`（折叠时） | Strategy 或 FormalStrategy | 可先粗表示为叶子；instances / observations 显式化后可直接 FormalStrategy |
| **`analogy`** | 单参数 `[p]`（折叠时） | Strategy 或 FormalStrategy | bridge claim 显式化后可直接 FormalStrategy |
| **`extrapolation`** | 单参数 `[p]`（折叠时） | Strategy 或 FormalStrategy | continuity claim 显式化后可直接 FormalStrategy |
...
**不适用于把整条归纳/溯因语义压平成单条 generic noisy_and。** 归纳、溯因、类比、外推在 theory 中都有各自的命名微观结构；如果要保留这些语义，应使用对应的命名 `type`，并在需要时展开为 FormalStrategy。CompositeStrategy 只是可选的组织层，不是这些类型的语义必需条件。
...
#### 3.5.2 CompositeStrategy（层次化组织）

将一个较大的推理组织为多个子策略。`sub_strategies` 可以包含任意形态（Strategy / CompositeStrategy / FormalStrategy），支持递归嵌套。

CompositeStrategy 的作用是**组织层次**，不是表达某一类 reasoning type 的唯一合法形式。典型用途：
...
当一条 Strategy 已经被展开为确定性 Operator skeleton 时，使用 FormalStrategy。`formal_expr` 只包含 Operator（确定性），不包含不确定的 ↝。

**关键约束：FormalExpr 只包含确定性 Operator，不包含概率参数。展开后的不确定性通过显式中间 Knowledge 的先验 π 表达（在 parameterization 层赋值）。**

### 3.6 命名策略的典型表示

#### 典型 fully expanded 形态 → FormalStrategy

所有命名策略在**未展开**时都可以先作为叶子 Strategy 存在。下面给的是它们在 **fully expanded** 状态下的典型 FormalStrategy 写法。
...
`abduction`、`induction`、`analogy`、`extrapolation` 虽然宏观上不是纯演绎，但在 theory 中都对应确定性的微观 skeleton。一旦 prediction / instance / bridge / continuity 等中间 claim 已显式化，它们也可以直接表示为 FormalStrategy。
...
#### 需要层次化组织时 → CompositeStrategy

CompositeStrategy 仍然有明确用途，但它描述的是**组织结构**而不是某个 type 的唯一形态。常见场景：
...
Strategy(type=infer)                          ← 初始：通用推理
  ...
  ├── reviewer fully expand → FormalStrategy + formal_expr
  ├── reviewer 为大型结构加层次 → CompositeStrategy + sub_strategies
  └── 保持 type=infer
```
...
7. `noisy_and` 仅用于前提联合必要的场景；对归纳、溯因、类比、外推等命名推理，应优先使用相应 `type`，而不是把整体语义压平成 generic `noisy_and`
...
- **FormalStrategy**：local 层和 global 层均可；当某条推理被 fully expand 为确定性 skeleton 时使用。
...
- **带隐式桥接/预测/实例的命名策略**（abduction, induction, analogy, extrapolation）：当 prediction、instance、bridge/continuity claim 等中间 Knowledge 已显式存在时，也可以生成 FormalExpr。是否额外包一层 CompositeStrategy 取决于是否需要层次化组织。
...
| type 与形态解耦 | type 表达推理语义家族，形态表达展开程度/组织方式。CompositeStrategy 是组织容器，不是某些 type 的唯一合法形式 |
