...
二者并非完全绑定；但对需要保留分解边界的策略族，顶层 canonical form 会默认保留为 `CompositeStrategy`。
...
| **`abduction`** | 单参数 `[p]`（折叠时） | CompositeStrategy（默认） | 顶层保留解释链边界；内部可从混合 children 演进到叶子全为 FormalStrategy |
| **`induction`** | 单参数 `[p]`（折叠时） | CompositeStrategy（默认） | 顶层保留多实例聚合边界；每个实例单元逐步 formalize |
| **`analogy`** | 单参数 `[p]`（折叠时） | CompositeStrategy（默认） | 顶层保留 bridge-based 迁移边界；leaf formalization 落在内部 skeleton |
| **`extrapolation`** | 单参数 `[p]`（折叠时） | CompositeStrategy（默认） | 顶层保留 continuity-based 延伸边界；leaf formalization 落在内部 skeleton |
...
**不适用于把整条归纳/溯因语义压平成单条 generic noisy_and。** 归纳、溯因、类比、外推在 theory 中都有各自的命名微观结构；如果要保留这些语义，应使用对应的命名 `type`，并沿着 `Strategy → CompositeStrategy（mixed）→ CompositeStrategy（all leaf FormalStrategy）` 的层次化路径 formalize。
...
CompositeStrategy 的作用是**保留分解边界**。对 `abduction`、`induction`、`analogy`、`extrapolation`、`toolcall`、`proof` 这类通常需要保留层次的策略族，它也是默认的顶层承载形态。

典型用途：

- 大型证明或工具调用的多步结构
- 只展开局部而保留其余部分折叠的 partial expansion
- 为 `abduction` / `induction` / `analogy` / `extrapolation` 保留语义上的组合边界
- 同一结论下对多条子路径做分组、审查或展示

对需要保留层次的策略族，典型演进有三个阶段：
...
对 `deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 这类简单确定性家族，顶层本身通常就可以直接是 FormalStrategy。对需要保留层次的策略族，FormalStrategy 通常是 CompositeStrategy 树中的叶子。
...
所有命名策略在**未展开**时都可以先作为叶子 Strategy 存在。下面先给出简单确定性家族的顶层 FormalStrategy 写法，再给出需要保留层次结构的策略族的 CompositeStrategy 写法。
...
`abduction`、`induction`、`analogy`、`extrapolation` 虽然宏观上不是纯演绎，但在 theory 中都对应确定性的微观 skeleton。为保留语义层次，它们的**顶层通常保持为 CompositeStrategy**；随着 formalization 推进，内部叶子逐步变成 FormalStrategy。

**溯因（abduction）**：`premises=[Obs], conclusion=H`

溯因的不确定性在于 H 和 prediction O 的先验，不在骨架中的 Operator。需要显式中间 claim：prediction `O`；`premises` 通常包含实际观测到的 claim（这里记为 `Obs`）。顶层保留 `abduction` 的层次边界。

```
CompositeStrategy(type=abduction, premises=[Obs], conclusion=H):
  sub_strategies:
...
归纳是多个 abduction-like 单元的并行重复。需要显式中间 claim：`Instance₁ ... Instanceₙ`。顶层保留“多实例聚合”为一个组合边界。
...
不确定性集中在 BridgeClaim 的先验 π(BridgeClaim)，不在骨架本身。顶层保留 bridge-based 迁移的组合边界。
...
与类比结构相同。不确定性在 ContinuityClaim 的先验。顶层保留 continuity-based 延伸的组合边界。
...
#### 需要保留 hierarchy 的策略族 → CompositeStrategy

CompositeStrategy 不只是“可选组织结构”，它也是需要保留分解边界的策略族的 canonical top-level form。常见场景：

- 很大的归纳或证明需要按章节/lemma 分组
- `toolcall` / `proof` 暂无稳定 canonical FormalExpr，需要先保留为多步结构
- 只想展开局部 skeleton，其余部分仍保留折叠
- 对 `abduction` / `induction` / `analogy` / `extrapolation` 显式保留顶层层次边界
...
Strategy(type=infer)                          ← 初始：通用推理
  ...
  ├── 对简单确定性家族可直接 fully expand → FormalStrategy + formal_expr
  └── 保持 type=infer
```
...
7. `noisy_and` 仅用于前提联合必要的场景；对归纳、溯因、类比、外推等命名推理，应优先使用相应 `type` 并保留其 CompositeStrategy 层次边界，而不是把整体语义压平成 generic `noisy_and`
...
- **带隐式桥接/预测/实例的命名策略**（abduction, induction, analogy, extrapolation）：当 prediction、instance、bridge/continuity claim 等中间 Knowledge 已显式存在时，会在其 CompositeStrategy 内部逐步生成 leaf FormalStrategy。顶层默认保留 CompositeStrategy 边界。
...
| type 与形态解耦 | type 表达推理语义家族，形态表达展开程度/组织方式。对需要保留层次的策略族，top-level canonical form 默认是 CompositeStrategy |
| FormalExpr 只包含 Operator | fully expanded 后的不确定性转移到显式中间 Knowledge 的 prior，FormalExpr 本身纯确定性，通常作为 CompositeStrategy 的 leaf 出现 |
