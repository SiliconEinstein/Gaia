...
type:
    ...
    infer                      # 默认，未分类推理（noisy-AND）
    soft_implication            # 单前提完整二参数模型
...
| conditional_probabilities 在 parameterization 层 | 概率参数不属于图结构；通过 StrategyParamRecord 存储，noisy-AND 每前提一个 qᵢ，soft_implication 两个参数 [p₁, p₂] |
