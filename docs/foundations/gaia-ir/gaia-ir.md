...
### 4.1 映射决策：内容去重与 CompositeStrategy 归类

规范化只做两件事：**Knowledge 内容去重**和**Strategy 归类**。不做独立性判断——那是 review 层的职责。

**Knowledge 去重（纯内容匹配）**

Knowledge ID = SHA-256(type + normalized_content)。同一命题无论谁推导、怎么推导，都绑定到同一个 global Knowledge：

| 情况 | 操作 |
|------|------|
| content hash 匹配已有 gcn | CanonicalBinding（绑定到已有 gcn） |
| 无匹配 | 创建新 gcn |

Knowledge 去重不关心该 Knowledge 在 local 图中的角色（premise / conclusion），只关心内容。

**Strategy 归类（CompositeStrategy 自动包裹）**

Knowledge 映射完成后，local Strategy 的 premises / conclusion 全部替换为 global ID。然后按 conclusion 归类：
...
| 该 conclusion 是新创建的 gcn | 作为普通 Strategy 添加 |

**关键规则：每个有多条推理路径的 conclusion 有且只有一个 CompositeStrategy 作为入口。** 所有指向该结论的 Strategy 都是它的 sub_strategies。

新 Strategy 没有 StrategyParamRecord → BP 不可见。新证据默认"待审"，直到 review 层赋予参数。

**独立性判断不在此层。** 两条推理链的 premises 相同或不同，都不能自动判定独立性——独立性取决于推理过程本身（隐藏在 Strategy 的 steps 中），只有 reviewer 能判断。

**语义重复的迟发现处理。** 措辞不同但语义相同的 Knowledge 会漏过 hash 去重——这是正常情况，由后续 curation 操作（merge）处理，不在 canonicalization 阶段解决。Merge 操作：重定向引用 → 暂停受影响 Strategy 参数 → re-review → 增量 BP。
...
**Content hash（唯一匹配方式）**：SHA-256(type + normalized_content)。完全相同的命题映射到同一个 global Knowledge，措辞不同的不匹配。

**不使用 embedding / TF-IDF。** Canonicalization 在 CI 中自动执行，CI 环境无法运行 embedding 模型。语义去重（措辞不同但含义相同）由后续 curation 操作处理。
...
CanonicalBinding:
    ...
    content_hash:           str    # SHA-256(type + normalized_content)
```

### 4.5 Strategy 提升与 CompositeStrategy 归类
...
5. **CompositeStrategy 自动归类**（见 §4.1）：按 conclusion 归类——若该 conclusion 已有其他 Strategy，自动包裹入 CompositeStrategy
...
**新 Strategy 默认 BP 不可见。** 新提升的 Strategy 没有 StrategyParamRecord，因此不参与 BP 计算。Review 层确认后赋予参数，Strategy 才对 belief 产生影响。
...
| independent_evidence / contradiction 用 Operator 表达 | 它们是确定性结构关系。equivalence Operator 连接等价 Knowledge；contradiction Operator 表达矛盾 |
| Canonicalization 纯内容去重 | Knowledge ID = SHA-256(type + content)，不用 embedding。语义去重由 curation merge 处理 |
| CompositeStrategy 自动归类 | 同一 conclusion 的所有 Strategy 归入一个 CompositeStrategy。独立性判断归 review 层 |
| 新 Strategy 默认 BP 不可见 | 无 StrategyParamRecord = 不参与 BP。Parameterization 层天然提供门控 |
