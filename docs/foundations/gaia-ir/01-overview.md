...
FormalStrategy 的确定性展开结构——由 Operator 列表构成。中间 Knowledge 不由 FormalExpr 自动创建，而需显式存在于图中；这些中间 claim 统一按 helper claim 规则管理。不是独立实体，而是 FormalStrategy 的嵌入字段。`deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 以及 `abduction`/`induction`/`analogy`/`extrapolation` 都可以直接表现为 FormalStrategy；如果需要保留更大的层次结构，则由外层 CompositeStrategy 组合这些子结构。`toolcall`/`proof` 暂无标准 FormalExpr 模板。Schema 见 [02-gaia-ir.md](02-gaia-ir.md) §3.2、§3.5 与 [04-helper-claims.md](04-helper-claims.md)。
