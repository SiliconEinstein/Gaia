...
Parameterization 是全局语义/贡献结构上的概率参数层。具体来说：

- `PriorRecord` 挂在 `GlobalSemanticGraph` 的 claim（`gcn_`）上
- `StrategyParamRecord` 挂在 `GlobalContributionGraph` 的 Strategy contribution（`gcs_`）上

它由一组**原子记录**构成——每条记录是一个 Knowledge 的 prior 或一个 Strategy contribution 的 conditional probabilities。不同 review 来源（不同模型、不同策略）产出不同的记录，BP 运行前按 resolution policy 组装成完整参数集。
...
StrategyParamRecord:
    strategy_id:            str              # 全局 Strategy contribution ID (gcs_ 前缀)
...
- **所有 global Strategy contribution 都有 conditional_probabilities**（折叠模式下的 ↝ 参数），因此所有推理类 `gcs_` 都需要 StrategyParamRecord。
...
- **一个 Knowledge/Strategy contribution 可以有多条记录**（来自不同 source），BP 运行时选择用哪条。
- **Cromwell's rule**：所有值钳制到 `[ε, 1-ε]`，ε = 1e-3。

## BP 运行时：Resolution Policy

BP 运行前，按 resolution policy 从原子记录中为每个 claim / Strategy contribution 选择一个值，组装成完整参数集：

| policy | 说明 |
|--------|------|
| **latest** | 每个 Knowledge/Strategy contribution 取最新的记录（按 `created_at`） |
...
BP 编译接受 `expand_set`（需要展开的 Strategy contribution ID 集合），根据 Strategy 形态和展开决策选择路径：

- **Strategy contribution 未在 expand_set 中 → 折叠**：使用 StrategyParamRecord.conditional_probabilities 编译为 ↝
...
- GlobalSemanticGraph 中每个 `type=claim` 的 Knowledge 都必须有对应的 PriorRecord
- GlobalContributionGraph 中所有**可运行的**推理类 Strategy contribution 都必须有 StrategyParamRecord（折叠参数）
...
每个 global claim Knowledge 的 prior 由 review 赋值。不存在聚合逻辑——Claim canonicalization 若命中已有 canonical claim，则复用该 `gcn_`；若未命中，则创建新的 `gcn_` 并由 review 赋值 prior。
