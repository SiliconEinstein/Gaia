...
| **命名策略**（`deduction`、`abduction`、`induction`、`analogy`、`extrapolation`、`reductio`、`elimination`、`mathematical_induction`、`case_analysis`） | `[p]`（1 个，折叠参数） | 折叠时视为单条 ↝ 的条件概率；展开后由 FormalExpr + 中间 Knowledge prior 承担细结构 |
...
| 命名策略（`deduction` 至 `case_analysis`） | Review 赋值折叠参数。展开时参数转移到中间 Knowledge 的 PriorRecord |
