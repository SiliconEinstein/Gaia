...
```
全局图（canonicalization 后）：
  Strategy_A (包 A): [P₁, P₂] → C    (数学推导, noisy_and, p=0.8)
  Strategy_B (包 B): [P₁, P₂] → C    (数值模拟, noisy_and, p=0.9)
                                  ↑ 同一个 global Knowledge
```

2. **Review 可选操作**：如果 reviewer 认为多条指向同一结论的 Strategy 应被视为"同一推理的不同实现"，可以将它们包装为 CompositeStrategy。这提供了多分辨率支持——折叠时视为一条 ↝，展开时看到各条路径。

```
全局图（review 组织后）：
  CompositeStrategy (review 创建): [P₁, P₂] → C
    sub_strategies:
      - Strategy_A (包 A): [P₁, P₂] → C  (数学推导)
      - Strategy_B (包 B): [P₁, P₂] → C  (数值模拟)
```

CompositeStrategy 包装是**可选的组织行为**，不是 canonicalization 的必要步骤。不包装也完全合法——多条独立 Strategy 指向同一 Knowledge 在概率推理中语义正确。
