...
### 4.1 映射决策：CanonicalBinding 与 Equivalence

规范化中最容易混淆的是两种都发生在“语义匹配命题”之间的机制：

- **CanonicalBinding（身份映射）**：local Knowledge 和 global Knowledge 是**同一个命题**的不同表示，且它们**不构成独立新增证据**。它是纯引用关系，不提供新证据，不创建新的图结构。
- **Equivalence Operator（等价声明）**：两个 global claim 在语义上等价，但其来源被 review / research agent 判定为**彼此独立的证据来源**，因此保留为两个 claim，再用 `equivalence` 建立显式结构关系。

Gaia IR 提供这种区分的结构基础。具体的语义匹配判定与“是否构成独立证据来源”的确认，由 review 服务和 agent 实现——IR 层不规定判定策略。

关键原则：**canonicalization 不能只看 local 图中的 premise / conclusion / background 角色；它必须先判断命题是否语义匹配，再判断来源是否独立。**

也就是说：

- 某个 local Knowledge 即使是某条 Strategy 的 conclusion，只要它与已有 global Knowledge 是同一个命题，且不构成独立新增证据，就应直接 CanonicalBinding 到该 global Knowledge
- 某个 local Knowledge 即使只是 premise，只要它与已有 global Knowledge 语义匹配，且并非独立证据来源，也应直接 CanonicalBinding
- 只有在“语义匹配 + 来源独立”同时成立时，才应保留为两个 global claim，并提 `equivalence` 候选

因此，当新包中的 local Knowledge 与全局图中已有 Knowledge 发生语义匹配时，review 层应在以下三种决策中选择：

**1. `match_existing`：语义匹配，且不构成独立新增证据**

如果 local Knowledge 与某个已有 global Knowledge 被判定为**同一个命题**，且其来源**不构成独立新增证据**，则直接创建 CanonicalBinding，复用该 global Knowledge。

典型情况：

- 同一 package 的反复更新、重编译或表述改写
- 没有任何独立 support 的 premise / background claim
- 只是局部 restatement、helper materialization 或格式差异

在这些情形下，新增的是引用或版本演化，不是新的证据线。
...
**3. `create_new_with_equivalence_candidate`：语义匹配，但来源独立**

如果 local Knowledge 与某个现有 global Knowledge 在语义上匹配，但 review 认为它们来自**彼此独立的证据来源**，则：

1. 先为该 local Knowledge 创建新的 global Knowledge
2. 再在新旧两个 global claim 之间提出 `equivalence` 候选

典型情况：

- 不同 package 基于各自独立的 supporting Strategies 得到相同结论
- 不同 review / research line 独立确认了同一命题
- 两条证据链在结构上足够独立，保留为两个 claim 比直接合并更能表达证据来源

在这些情形下，“新增证据”不应被静默塌缩进已有 claim；它应以独立 claim + `equivalence` 的形式保留下来。

### 4.1.1 为什么不能按 premise / conclusion 角色分流

如果 canonicalization 取决于 local 角色，而不是取决于“语义匹配 + 证据独立性”，就会产生两个问题：

1. **本不独立的重复表述被错误保留为两个 global 节点**
   例如同一 package 的连续更新或无 support 的 premise，只因出现在 conclusion 位置，就被错误保留成独立 claim。

2. **真正独立的证据来源被错误塌缩**
   如果只因为某个命题在 local 图里是 premise，就一律直接 merge，也会丢失“独立来源应保留为单独 claim”的信息。

因此，是否复用已有 global Knowledge，不应由 local 角色单独决定；它应由**命题身份 + 来源独立性**共同决定。
...
CanonicalBinding:
    ...
    package_id:             str
    version:                str
    decision:               str    # "match_existing" | "create_new" | "create_new_with_equivalence_candidate"
...
如果多个 local 包都指向语义相同的命题，规范化时有两种可能：

- 若 review 认为这些来源并不独立，则它们应共享同一个 global conclusion claim，各自独立的 global Strategy 一并指向该 claim
- 若 review 认为这些来源彼此独立，则它们应保留为多个 global claim，并通过 `equivalence` 建立结构关系

换句话说，“新增证据”有时体现在多个 Strategy 指向同一 claim，有时体现在多个等价 claim 并存；关键分界线是**来源是否独立**。
...
| canonicalization 先看命题身份，再看来源独立性 | 语义匹配后，不独立的重复来源走 CanonicalBinding；独立来源保留为单独 claim，并提 `equivalence` 候选。不能只按 premise / conclusion 角色分流 |
