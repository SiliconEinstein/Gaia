...
- **CanonicalBinding（身份映射）**：local Knowledge 和 global Knowledge 是**同一个命题**的不同表示。它是纯引用关系，不提供新证据，不创建新的图结构。
- **Relation Operator（关系声明）**：两个独立的 global claim 之间存在某种结构关系，如 `equivalence`、`contradiction`、`complement`。它是图中的显式结构，而不是 dedupe 补丁。

Gaia IR 提供这种区分的结构基础。具体的匹配判定与关系确认由 review 服务和 agent 实现——IR 层不规定判定策略。

关键原则：**canonicalization 按命题身份处理，而不是按 local 图中的 premise / conclusion / background 角色处理。**

也就是说：

- 某个 local Knowledge 即使是某条 Strategy 的 conclusion，只要它与已有 global Knowledge 被判定为同一个命题，就应直接 CanonicalBinding 到该 global Knowledge
- 某个 local Knowledge 即使只是 premise，只要它不是现有 global Knowledge 的同一命题，也不应因“只是前提”而被静默合并

独立推理来源带来的“新增证据”体现在：

- 新的 global Strategy 指向同一个 global conclusion claim
- 新增的 helper claim / Operator / refs / review 记录

而不是默认通过“重复创建 claim 节点 + 再补一个 equivalence”来表达。
...
- 这与 local Knowledge 在图中充当 premise、conclusion、background 还是 helper claim 无关
- 若 local conclusion 被绑定到已有 global claim，则其独立推理来源通过后续提升的 global Strategy 表达，而不是通过复制 claim 节点表达
...
**3. `create_new_with_relation_candidate`：不是同一命题，但存在待确认关系**

如果 review 认为 local Knowledge **不是**某个现有 global Knowledge 的同一命题，但两者之间可能存在显式结构关系，则：

1. 先为该 local Knowledge 创建新的 global Knowledge
2. 再为相关 global claim 之间提出关系候选（最常见的是 `equivalence`，也可扩展到 `contradiction` / `complement`）

这里的关系候选是**附加结构**，不是 identity matching 的替代品。

### 4.1.1 为什么不能按 premise / conclusion 角色分流

如果 canonicalization 取决于 local 角色，就会产生两个问题：

1. **同一命题被拆成两个 global 节点**
   仅因为它在某个包里恰好作为 conclusion 出现，就新建 global 节点，再用 equivalence 补回去。

2. **equivalence 被降格为 dedupe 补丁**
   但在 Gaia IR 中，`equivalence` 是真正的结构关系，应该只在 review 确认两个独立命题具有等价关系时才出现。
...
CanonicalBinding:
    ...
    package_id:             str
    version:                str
    decision:               str    # "match_existing" | "create_new" | "create_new_with_relation_candidate"
...
如果多个 local 包独立推出同一个命题，且这些 local conclusion 在规范化时都被判定为同一个 global claim，那么：

- 它们应共享同一个 global conclusion claim
- 各自独立的 global Strategy 一并指向该 claim
- “新增证据”来自这些 Strategy 的并存，而不是来自 duplicate claim + equivalence
...
| canonicalization 按命题身份而非 local 角色 | 是否复用已有 global Knowledge 只由 identity matching 决定；独立推理来源在 Strategy 层体现，不靠 duplicate claim + equivalence 补丁 |
