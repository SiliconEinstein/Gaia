...
**Strategy 的处理：合并为 CompositeStrategy**

当新包的 Strategy（提升到全局后）与已有 Strategy 共享相同前提和结论时，**必须合并为 CompositeStrategy**，不能让多条独立 Strategy 并列指向同一 Knowledge。

理由：如果两条 Strategy 用相同前提指向同一 Knowledge 但作为独立因子存在，概率推理会将同一组证据计算两次（double counting）。合并为 CompositeStrategy 后，折叠时只有一个因子（一组条件概率），避免重复计算。
