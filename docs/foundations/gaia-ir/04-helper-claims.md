...
**不折叠意味着什么？** FormalStrategy 的内部结构直接暴露在图中：每个 Operator 成为图中一个确定性约束节点，每个中间 claim（如果承载不确定性）作为显式变量节点。推理引擎在这个展开后的图上工作——不需要计算等效条件概率 P(conclusion | premises)，因为每个内部步骤都是可见的。

例如，`FormalStrategy(type=analogy, premises=[SourceLaw, BridgeClaim], conclusion=Target)` 展开后，图中有：

```
conjunction([SourceLaw, BridgeClaim], conclusion=M)   ← 确定性约束
implication([M], conclusion=Target)                     ← 确定性约束
```

如果 M 是私有的，可以把整个 FormalStrategy 折叠成 P(Target | SourceLaw, BridgeClaim)。但如果 M 被提升为公共 claim（比如另一个 Strategy 把 M 放进了 premises），就不能折叠——M 必须保留在图中作为一个可见的变量节点，推理引擎直接处理 M 和两个 Operator 约束
