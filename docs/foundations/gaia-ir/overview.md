...
`type` 表示**推理语义家族**，`形态` 表示**展开程度/组织方式**。除 `toolcall` / `proof` 目前默认使用 `CompositeStrategy` 外，绝大多数 type 都不强制绑定某一种形态。
...
| **abduction / induction / analogy / extrapolation** | `[p]`（折叠时） | Strategy / FormalStrategy / CompositeStrategy | 可先粗表示为叶子；中间 claim 显式化后可直接 FormalStrategy；大结构可再加 Composite 层次 |
...
FormalStrategy 的确定性展开结构——由 Operator 列表构成。中间 Knowledge 不由 FormalExpr 自动创建，而需显式存在于图中。不是独立实体，而是 FormalStrategy 的嵌入字段。`deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 通常有直接模板；`abduction`/`induction`/`analogy`/`extrapolation` 在 prediction、instance、bridge/continuity claim 等中间节点显式化后也可写成 FormalExpr。`toolcall`/`proof` 暂无标准 FormalExpr 模板。Schema 见 [gaia-ir.md](gaia-ir.md) §4。
