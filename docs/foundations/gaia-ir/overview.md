...
`type` 表示**推理语义家族**，`形态` 表示**展开程度/组织方式**。二者不是同一个维度；但对需要保留分解边界的策略族，顶层通常会默认保留为 `CompositeStrategy`。
...
| **abduction / induction / analogy / extrapolation** | `[p]`（折叠时） | CompositeStrategy（默认） | 顶层保留层次结构；内部从 `Strategy + FormalStrategy` 混合态逐步演进到叶子全为 `FormalStrategy` |
...
FormalStrategy 的确定性展开结构——由 Operator 列表构成。中间 Knowledge 不由 FormalExpr 自动创建，而需显式存在于图中。不是独立实体，而是 FormalStrategy 的嵌入字段。`deduction`/`reductio`/`elimination`/`mathematical_induction`/`case_analysis` 常直接表现为顶层 FormalStrategy；`abduction`/`induction`/`analogy`/`extrapolation` 通常把 FormalExpr 放在保留层次边界的 CompositeStrategy 叶子上。`toolcall`/`proof` 暂无标准 FormalExpr 模板。Schema 见 [gaia-ir.md](gaia-ir.md) §4。
