...
除了对象 `id` 之外，Knowledge 还可以带一个跨包稳定的 `content_hash`。`id` 回答“这个节点是谁”，`content_hash` 回答“它代表的内容是什么”。完整规则见 [identity-and-hashing.md](identity-and-hashing.md)。
...
`Knowledge.content_hash` 独立于 `id`：相同内容的 local 节点可以有不同 `lcn_id`，但共享同一个 `content_hash`。规范化（lcn → gcn 映射）见 [canonicalization.md](canonicalization.md)，完整身份规则见 [identity-and-hashing.md](identity-and-hashing.md)。
