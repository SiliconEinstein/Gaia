...
Operator 分为两类，`conclusion` 与 `variables` 的关系由类别决定：

| 类别 | Operator 类型 | conclusion 位置 |
|------|-------------|----------------|
| **Directed**（有向） | `implication`, `conjunction` | `conclusion = variables[-1]`，是输出变量 |
| **Relation**（关系） | `equivalence`, `contradiction`, `complement`, `disjunction` | `conclusion` 是结构型 helper claim，**不在** `variables` 中 |
...
4. **Directed operator**：`conclusion = variables[-1]`
5. **Relation operator**：`conclusion` 是标准 helper claim，且**不在** `variables` 中。`conclusion` 应语义上对应其结构型 helper claim，不允许借此手写任意主观结论
...
9. 命名策略（deduction 至 case_analysis）以 leaf Strategy 形态存在时，视为过渡态——推理本身不需要用户指定条件概率，但尚未完成 FormalExpr 还原。BP 将其视为隐含 `p = 1.0 - ε` 的确定性推理。未来应进一步还原为 FormalStrategy
10. FormalExpr 的内部中间节点默认不作为外部 Strategy 的 `premises`；若需要跨 Strategy 共享，必须提升为显式公共 Knowledge
11. helper claim 仍然是 `claim`，不引入新的 Knowledge primitive；当前 helper claim 术语只用于结构型 result claim，并额外区分 public/private 角色
