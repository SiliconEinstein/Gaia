...
Setting 不携带 prior，不参与 BP。可作为 factor 的承载性前提（structural dependency），但不创建 BP 边。对齐 [04-reasoning-strategies.md §1.2](../theory/04-reasoning-strategies.md) 和 [02-gaia-ir.md §1.2](../gaia-ir/02-gaia-ir.md)。
...
Equivalence 同理（`reasoning_type: equivalent`）。参见 `gaia-ir/02-gaia-ir.md` 中 FactorNode schema 的定义。
