...
Setting 不携带 prior，不参与 BP。可作为 factor 的承载性前提（structural dependency），但不创建 BP 边。对齐 [05-science-ontology.md §1.2](../theory/05-science-ontology.md) 和 [graph-ir.md §1.2](../graph-ir/graph-ir.md)。
...
`from:` 创建的粗因子，其多个前提遵循**合取语义**（联合必要条件）。

论证策略（`#abduction` 等）生成细因子图，由 entailment + equivalence + contradiction 组合而成，所有因子 p=1——推理效果通过 BP 消息传递协作实现。

参见 [03-coarse-reasoning.md](../theory/03-coarse-reasoning.md) 和 [06-science-formalization.md §2.4](../theory/06-science-formalization.md)。
...
**粗因子图：** `[] → H`（无合取 premise，H 是 conclusion）
...
**粗因子图：** `[A₁, A₂, A₃] → B`（实例是 premise，定律是 conclusion，合取语义）
