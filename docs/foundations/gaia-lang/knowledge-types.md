...
- **参与 BP**：否。Setting 不携带 probability（无 prior、无 belief），不参与 BP 消息传递。参见 [05-science-ontology.md §1.2](../theory/05-science-ontology.md) 和 [graph-ir.md §1.2](../graph-ir/graph-ir.md)。
...
`from:` 创建的粗因子，其多个前提遵循**合取语义**（联合必要条件）：所有前提必须同时成立，结论才获得支撑。任何一个前提失败，整条推理链断裂。参见 [03-coarse-reasoning.md](../theory/03-coarse-reasoning.md)。

论证策略（`#abduction`、`#induction` 等）生成的**细因子图**由 entailment + equivalence + contradiction 因子组合而成，所有因子 p=1——推理效果通过 BP 消息传递协作实现。参见 [06-science-formalization.md §2.4](../theory/06-science-formalization.md)。
...
- **Theory 层**：[05-science-ontology.md](../theory/05-science-ontology.md)（知识类型定义）、[03-coarse-reasoning.md](../theory/03-coarse-reasoning.md)（合取语义、粗/细因子图）、[06-science-formalization.md](../theory/06-science-formalization.md)（形式化工作流 §2.4）
...
- `docs/foundations/theory/05-science-ontology.md` —— 本体论分类
