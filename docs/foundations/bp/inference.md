...
2. **Relation conclusion**（EQUIVALENCE/CONTRADICTION/COMPLEMENT/IMPLICATION 的 conclusion）→ `add_evidence(1)`（strict hard evidence）；graph relation assertion 优先于 `node_priors`
...
这确保了 Jaynes 严格语义：
- **Class I (Hard Evidence)**：`add_evidence(var, {0, 1})`。Gaia 把它作为严格 δ 约束处理：不一致赋值权重为 0（exact/JT 中是 `-inf` log weight；消息传递路径以 delta message 表示）。只有硬约束且没有 soft prior 时，posterior 就是在所有可行 joint assignments 上均匀计数。
- **Class IV (Unary Priors)**：`observe()` 或 `node_priors`，观测到的不确定信息
- **Class V (MaxEnt Free)**：其他所有待推断变量，默认 0.5

### Cromwell's rule

所有 unary prior 和 soft probability 参数都被钳制到
`[epsilon, 1 - epsilon]`，其中 `epsilon = 1e-3`（见
`factor_graph.py:CROMWELL_EPS`）。确定性 operator potential 本身仍是 strict
0/1 truth table，hard evidence 也是 strict 0/1。Cromwell clamp 防止 soft
prior / empirical CPT 侧出现不可更新的绝对 0 或 1；它不软化 Class-I 逻辑
约束。

**实现位置**：
- `factor_graph.py::add_variable` — unary priors 使用 Cromwell clamp
- `factor_graph.py::add_evidence` — 写入 strict hard evidence，并把 display value 设为 `0.0` / `1.0`
- `trw_bp.py` / `bp.py` — hard evidence 发送 delta messages
- `junction_tree.py` / `exact.py` — 不一致 hard assignments 使用零权重（`-inf` log weight）
- `mean_field.py` — hard-evidence variables 固定为 `0.0` / `1.0`，并从 soft-var 列表中排除
...
`{0, 1}`；Cromwell clamp 用在 unary prior 和 soft 参数上。Relation operator
（implication / equivalence / contradiction / complement）的 conclusion 使用
`add_evidence(1)`（断言关系成立），约束自然激活。Expression operator
（conjunction / disjunction / negation，以及 formula-internal connective
implication）的 conclusion 使用 π = 0.5（计算输出），belief 由 variables 决定。详见
