...
- **Class V (MaxEnt Free)**：其他所有待推断变量，没有 synthetic unary
  prior；在无约束二值计数测度下边际为 0.5。

### Cromwell's rule

所有 unary prior / evidence、soft probability 参数，以及运行时确定性张量
都被钳制到
`[epsilon, 1 - epsilon]`，其中 `epsilon = 1e-3`（见
`factor_graph.py:CROMWELL_EPS`）。逻辑层真值表仍可写成 strict 0/1；进入
exact、junction-tree 和 contraction 数值路径时，确定性 factor 的 `0` 会变成
`ε`，`1` 会变成 `1-ε`。Cromwell clamp 防止出现不可更新的绝对 0 或 1，并
保持推理路径在数值上可比较。
...
CONTRADICTION / COMPLEMENT）。逻辑真值表是 strict delta `{0, 1}`；当前
推理后端在 exact、junction-tree 和 contraction 路径中把 deterministic
factor 数值软化为 `{ε, 1-ε}`。
