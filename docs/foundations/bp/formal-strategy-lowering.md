...
priors 和 soft probability 参数，不用于 deterministic truth table 或 hard
evidence。
...
**implication / equivalence / contradiction / complement**：conclusion $H$ 表达一个**关于相关变量关系的独立断言**。

$H = (A \leftrightarrow B)$ 说的是 "A 和 B 真值一致"——这个信息**无法从** $\pi(A)$ 和 $\pi(B)$ 推导出来。它是关于 A、B 相关性的新信息。

因此 lowering 对 $H$ 调用 `add_evidence(H, 1)`：**算子的存在本身就是断言 "此关系成立"**。
...
$M = A \wedge B$ 可以从 $\pi(A)$ 和 $\pi(B)$ 直接算出（在独立假设下）。若把 $M$ 也 hard-assert 为真，会引入与 $\pi(A)$、$\pi(B)$ 重复的信息。
...
**当错误地把 $M$ hard-assert 为真时：**

$$\text{belief}(M\!=\!1) = 1$$

$M$ 的 belief 恒定接近 1，不随 A、B 变化——退化为常数，不再是计算结果。✗

对比 equivalence $f(A, B, H)$，`add_evidence(H, 1)`：

$$\text{belief}(H\!=\!1) = 1 \quad \text{（正确：断言"关系成立"，约束 A=B）}$$

| 先验 | conjunction belief(M) | equivalence belief(H) | 角色 |
|------|----------------------|----------------------|------|
| $0.5$ | $ab$（计算） | $ab + (1-a)(1-b)$（计算） | 推理输出 |
| hard evidence | $1$（常数） | $1$（常数） | 断言 |

Conjunction 需要 $\pi = 0.5$ 才能做有意义的计算；relation operator 需要 hard evidence 才能激活约束。
...
$$\sum_H \pi(H) \cdot f(A, B, H) = \pi(H\!=\!1) \cdot \mathbf{1}[\text{relation holds}] + \pi(H\!=\!0) \cdot \mathbf{1}[\text{relation fails}]$$

**断言型 `add_evidence(H, 1)`：**
...
$$0.5 \cdot \mathbf{1}[\text{relation holds}] + 0.5 \cdot \mathbf{1}[\text{relation fails}] = 0.5 \quad \forall (A, B)$$
...
equivalence([D, Obs]) → Eq         # Eq: hard evidence，断言型
```
...
equivalence([D, Exh]) → Eq              # Eq: hard evidence，断言型（dead-end，约束保持）
contradiction([C₁, E₁]) → Contra₁       # Contra₁: hard evidence，断言型（dead-end，约束保持）
contradiction([C₂, E₂]) → Contra₂       # Contra₂: hard evidence，断言型（dead-end，约束保持）
...
以下策略的 FormalExpr 主要使用 conjunction + implication。Conjunction helper 是计算型中间变量（$\pi = 0.5$，连接两个因子）；deduction 等 hard implication 的 relation helper 使用 hard evidence 断言为真。`support` 是例外：它把 implication warrant prior 折成 `SOFT_ENTAILMENT`，而不是 hard assertion。

| 策略 | FormalExpr 结构 | Helper 类型 |
|------|----------------|------------|
| deduction | conjunction(premises) → C, implication(C, concl) | conjunction helper 计算；implication helper hard evidence |
| analogy | conjunction(premises) → C, implication(C, concl) | conjunction helper 计算；implication helper hard evidence |
| extrapolation | conjunction(premises) → C, implication(C, concl) | conjunction helper 计算；implication helper hard evidence |
| mathematical_induction | conjunction([base, step]) → C, implication(C, law) | conjunction helper 计算；implication helper hard evidence |

注：deduction 单前提时跳过 conjunction，但仍会产生并 hard-assert implication relation helper。
...
for op in formal_expr.operators:
    ...
    add_factor(fid, operator_to_factor_type(op.operator), op.variables, op.conclusion)
    if operator_asserts_relation(op):
        add_evidence(op.conclusion, 1)
```
...
|--------------|----------------|------|
| Relation（equivalence, contradiction, complement, implication warrant） | hard evidence | 断言：算子的存在 = 关系成立 |
| Expression（negation, conjunction, disjunction） | $0.5$ | 计算：belief 由 variables 决定 |

判定规则：conclusion 的 $P(H\!=\!1)$ 能否从 $\pi(\text{variables})$ 推导出来？能 → 0.5（计算型）；不能 → hard evidence（断言型）。

### 7.3 与势函数和推理文档的关系

- 势函数细节见 [`potentials.md`](potentials.md)：每种 deterministic FactorType 的势函数是 strict 0/1；Cromwell clamp 仅作用于 unary prior 和 soft probability 参数（例如 ↝ 的 `p₁ / p₂`），不出现在 deterministic potential 或 hard evidence 中。
- Relation operator 的 conclusion 通过 §7.2 的 hard evidence 自然激活约束，没有单独的门控变量；推理流程见 [`inference.md`](inference.md)。
