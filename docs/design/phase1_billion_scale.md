...
| 变更记录 | v1.1: 按 API v2 重构 §3/§5/§7; v1.2: 存储架构重构为 Neo4j+LanceDB+ByteHouse 三层，去除 DuckDB 和 Redis |
...
1. **API 架构**：采用两层 API 设计（详见 `docs/plans/2026-03-02-lkm-api-design-v2.md`）。Layer 1 (Knowledge Graph API) 提供 Git-like commit 写入模型（commit → review → resolve → merge）、精确读取和多路搜索；Layer 2 (Research API) 在 Layer 1 基础上编排 novelty/reliability/contradiction 等研究级查询。所有外部写入通过 commit 工作流入图，确保知识质量。
...
| **Layer 1 Search QPS** | 200 QPS | /search/propositions 多路召回 |
...
| **GET /propositions/{id}** | < 5ms | < 50ms | 强一致点查 |
| **POST /search/propositions** | < 100ms | < 500ms | 多路召回 |
...
graph TD
    ...
    subgraph API["API Gateway (FastAPI)"]
        subgraph L1["Layer 1: Knowledge Graph API"]
            ...
            GR["Graph Read API<br/>/propositions, /edges"]
            SA["Search API<br/>/search/propositions, /search/reasoning"]
        end
...
- **外部写入路径**：所有外部写入（Research Agent 提交新发现、Paper Ingest Agent 批量注入）通过 Layer 1 的 Commit API 进入系统，经过 commit → review → resolve(可选) → merge 四步工作流后入图。详见 API v2 文档。
...
| **Commit Engine** | Git-like 写入工作流：commit → review → resolve → merge | Python + LLM API |
...
propositions_schema = pa.schema([
    ...
    # 状态与时间
    pa.field("status", pa.utf8()),                       # active | merged | deleted
...
class VectorSearchClient(ABC):
    ...
    @abstractmethod
    def insert(self, prop_id: int, embedding: list[float]) -> None: ...

    @abstractmethod
    def search(self, query: list[float], k: int = 50) -> list[tuple[int, float]]:
        """返回 [(prop_id, distance), ...]"""
        ...
...
| **L2 Cold** | 历史轮次 Lance segments、已 merge 的节点、LLM 调用日志 | SATA SSD / HDD | ~15 TB | < 100ms |
...
**自动化分层规则**：
- 节点 status = "merged" 且 inactive_round > 当前 round - 10 → compaction 到独立 segment 后降级到 L2
...
> **与 API v2 的对齐说明**：API v2 确立了 Git-like commit 写入模型，所有外部写入通过 commit → review → resolve → merge 工作流入图。本节描述的内部 Build Pipeline（Join/Meet/Verify）是系统自主的知识发现流程，由 merge 操作异步触发。
...
| **纠错/合并** | Knowledge Curator | ✅ 通过 `/commits` | modify_edge, merge_propositions 等操作 |
...
graph LR
    ...
    G --> H["Commit Engine<br/>(轻量检查 → LLM review<br/>→ resolve → merge)"]
...
- `lkm_commits_resolve_total` (counter, label: action=reuse|keep_new)
...
│          /commits/{id}/resolve, /commits/{id}/merge,  │
│          /commits/batch                               │
│  Read:   GET /propositions/{id}, /edges/{id},         │
│          /propositions/{id}/subgraph                  │
│  Search: POST /search/propositions,                   │
│          /search/reasoning, /search/subgraph          │
│  （详见 API v2 文档）                                  │
└──────────────────────────────────────────────────────┘
                        │
                   共享存储层
...
| `/commits/{id}/resolve` | POST | 整改重合命题 | 低 | 状态更新 + 引用校验 |
| `/commits/{id}/merge` | POST | 入图 + 触发 BP | 中 | 图写入 → 局部 BP → 异步索引更新 |
| `/commits/batch` | POST | 批量提交多个 commit | 中 | 批量优化：跨 commit 去重、批量 LLM review |
| `/commits/batch/{batch_id}` | GET | 批量提交进度 | 低 | 读取 batch 状态 |
| **精确读取** | | | | |
| `/propositions/{id}` | GET | 命题详情 | 低 | LanceDB 点查（含 belief） |
| `/edges/{id}` | GET | 超边详情 | 低 | Neo4j 点查 |
| `/propositions/{id}/subgraph` | GET | 命题邻居子图 | 低 | Neo4j 图遍历 (h-hop Cypher) |
| **检索** | | | | |
| `/search/propositions` | POST | 找相似命题 | 低 | ByteHouse 向量搜索 + BM25 + Neo4j 图拓扑多路召回 |
| `/search/reasoning` | POST | 找相似推理链 | 低 | 基于 reasoning 文本的多路召回 |
| `/search/subgraph` | POST | 找相似子图 | 中 | 命题检索 + 子图模式匹配 |
...
graph TD
    ...
    subgraph REVIEW["POST /commits/{id}/review"]
        ...
        B3 --> B4{"有重合?"}
        B4 -->|"有"| B5["status → needs_resolution"]
        B4 -->|"无"| B6["status → reviewed (pass)"]
    end

    subgraph MERGE["POST /commits/{id}/merge"]
        C1["校验 status = reviewed|resolved"] --> C2["创建新节点/边<br/>(LanceDB + Neo4j + ByteHouse)"]
        C2 --> C3["更新 Neo4j 图拓扑"]
...
```mermaid
graph TD
    Q["POST /search/propositions<br/>{text, filters}"] --> EMB["Embedding 计算<br/>(本地 GPU, ~50ms)"]
    ...
    MERGE --> FILTER["应用 filters<br/>(status, type, paper_id,<br/>min_belief, min_quality)"]
...
**特殊查询模式**：
- 当指定 `paper_id` 且不提供 `text` 时：跳过向量搜索，直接通过 Neo4j 查询该论文的所有命题
- 当指定 `min_belief` 时：从 LanceDB `propositions.belief` 列过滤
...
| GET /propositions/{id} | 5ms | 10ms | 50ms | 强一致点查 |
| POST /search/propositions | 100ms | 200ms | 500ms | 多路召回 |
| **Layer 2（待详细设计）** | | | | |
...
| Layer 1 Graph Read API (GET /propositions, /edges, /subgraph) | 后端组 | 通过现有单元测试 + API v2 规范 |
...
| 交付物 | 负责人 | 完成标准 |
|--------|--------|---------|
| **Layer 1 Commit API** (commit/review/resolve/merge) | 后端组 | 单 commit 全流程 end-to-end |
