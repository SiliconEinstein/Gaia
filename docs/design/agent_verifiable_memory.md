...
| 版本 | 1.0 |
| 日期 | 2026-03-04 |
| 关联文档 | [phase1_billion_scale.md](phase1_billion_scale.md), [scaling_belief_propagation.md](scaling_belief_propagation.md) §2, [theoretical_foundations.md](theoretical_foundations.md) |
| 状态 | Wishlist |
...
**类比 Git**：Git 不管理 IDE 的 undo history。Agent 先在自己的 workspace 里理清推理链，然后一次性提交为正式 commit。Gaia 提供只读的 dry-run API 供 agent 在 workspace 中调用，但不托管 workspace 本身。

```
Git 类比：
  git repo   = 已提交的代码        ←→  Gaia graph  = 已验证的知识
  IDE/编辑器 = 开发者的工作区      ←→  Agent workspace = agent 的思考过程
  git diff   = 对比工作区和 repo   ←→  dry-run API = 对比草稿和已知知识

  Git 不管理编辑器。Gaia 不管理 agent 的思考过程。
  Git 提供查询工具给编辑器用。Gaia 提供验证 API 给 agent 用。
```
