...
> **Status:** Draft
> **Date:** 2026-03-31
> **实现文件:** target external LKM implementation (`gaia_lkm/models/`), not
> present in the current Gaia v0.5 CLI repository.

## 概述

M1 定义 LKM 自有的 Pydantic v2 models。这些 models 是 LKM 内部存储格式，与上游 `gaia.engine.ir.*` 完全独立——后者是 ingest 时的输入，不是 LKM 存储格式。
...
- `Gaia/gaia/engine/ir/knowledge.py`：Knowledge、content_hash 实现参考
- `Gaia/gaia/engine/ir/strategy.py`：Strategy / FormalStrategy 结构参考
- `Gaia/gaia/engine/ir/graphs.py`：LocalCanonicalGraph / ir_hash 实现参考
- `Gaia/gaia/engine/ir/parameterization.py`：PriorRecord / ResolutionPolicy 实现参考
