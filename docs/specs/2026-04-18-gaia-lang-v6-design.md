...
3. **无 Action IR / 无额外调用层**：`Claim.supported_by(...)`、`Claim.derived_from(...)`、`match()`、`contradict()`、`compute()` 等只是 DSL 层 constructor。编译后只保留 Gaia IR 的 Knowledge、Strategy、Operator 和 parameterization records。
...
| `claim.derived_from(...)` | 确定性推导 shortcut，创建 `prior=1` 的 derivation Warrant | `Warrant(pattern="derivation")` + `Strategy(type="support")` |
...
但它的 review 语义是 binary/structural：接受则保留 `prior=1`，不接受则应打回修改，或改写成普通 `supported_by(pattern="derivation", prior=...)` 来表达不确定的经验性支持。当前 Gaia IR parameterization 会按 Cromwell's rule 将持久化的 `PriorRecord(1)` clamp 到 `1-ε`；这是存储层约束，不改变 v6 authoring surface 的语义。
...
| `Warrant` from `derived_from(...)` | 固定 `PriorRecord(knowledge_id=warrant_id, value=1)`；持久化时由 IR parameterization clamp 到 `1-ε` |
...
| `claim.derived_from(...)` | 创建 `Warrant(pattern="derivation", prior=1)` + `Strategy(type="support", premises=[warrant, *inputs], conclusion=claim)` |
