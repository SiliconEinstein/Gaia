...
class Action:
    ...
    warrants: list[Claim] = []     # generated helper claims (review focus)
```

`warrants` tracks the Operator-level helper Claims generated by this Action (e.g., `Implies(AllTrue(A,B), C)` for derive). These are the audit targets in ReviewManifest.
