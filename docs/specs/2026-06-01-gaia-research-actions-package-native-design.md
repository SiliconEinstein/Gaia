...
## 11. Gaia Package 里的 Knowledge Model
...
`undercuts`、`replicates`、`extends`、`background_for`、`needs_more_evidence`。
这些关系不应该全都变成 BP factor 或 hard logical operator。

### 11.1 Focus 通常应该是 Question，不是 Claim
...
### 11.2 Obligation 是过程知识，不是 stable truth claim
...
### 11.3 Assessment relation 比 formal relation 更丰富
...
| `replicates` | 不同 paper/model/dataset 得到类似结果 | assessment artifact，提升 confidence |
| `extends` | evidence 扩大适用范围 | assessment artifact 或新 claim |
...
### 15.1 Inquiry obligation metadata
...
### 15.2 Inquiry focus over freeform research questions
...
### 15.3 Assessment relation vocabulary

当前 `candidate_relation(...)` 只支持 `equal`、`contradict`、`exclusive`。Assess
需要非 lowering 的 relation types，例如 `support`、`oppose`、`qualify`、
`undercut`、`replicate`、`extend`、`background`。
...
promotion / LKM edges 对齐。

### 15.4 Question targets and answer claims
...
### 15.5 Search result artifact standardization
...
### 15.6 Assessment artifact schema
...
### M1: Package-native research layout

添加 `.gaia/research/` helpers 和 manifest/event utilities。
...
1. `gaia research` 是否应立即放进主 Gaia CLI，还是先作为内部模块，M1 之后再暴露？
2. `.gaia/research/` 只用 JSON 文件，还是为了大规模检索/审查加 SQLite 或 index？
3. accepted open questions 什么时候用 `gaia author question`，什么时候只用
   `gaia inquiry obligation add`？
4. assessment artifacts 是否参与 `gaia build check`，还是应该有单独的
   `gaia research check`？
5. assessment relation vocabulary 应该只是 artifact schema，还是也应该成为 Gaia DSL
   scaffold？
6. `question(...)` 是否需要 first-class alternatives / answer claims，还是现有
   `targets=[...]` 加 CLI 支持就够？
