...
1. 必须是合法 JSON
2. 顶层必须是 object
3. 每个 value 必须有 `type` 字段，且 type 必须是 CSL 1.0.2 合法 type
4. 每个 value 必须至少有 `title` 字段（最小可渲染要求）
5. 可选但推荐的字段：`author`、`issued`（出版日期）、`DOI` 或 `URL`

缺失字段不 error，只 warning —— CSL 样式会自己处理缺失字段。
