...
├── FormalSupport
│   ├── DeductionSupport
│   ├── AbductionSupport        # observation, alternative
│   ├── AnalogySupport          # source, bridge
│   ├── ExtrapolationSupport    # source, continuity
...
**基数说明：一个 Claim 只挂一条 Support。** 如果同一个结论需要多条独立的 support 路径，应使用 `CompositeSupport` 聚合：
...
### 4.1 FormalSupport 及其子类

有 canonical skeleton 的 support，核心语义由结构决定。每个 formal family 是一个独立子类，用类型安全字段表达语义角色：

| 子类 | 语义角色字段 |
|------|------------|
| `DeductionSupport` | `given: list[Claim]` |
| `AbductionSupport` | `observation: Claim`, `alternative: Claim \| None` |
| `AnalogySupport` | `source: Claim`, `bridge: Claim` |
| `ExtrapolationSupport` | `source: Claim`, `continuity: Claim` |
| `EliminationSupport` | `exhaustiveness: Claim`, `excluded: list[tuple[Claim, Claim]]` |
| `CaseAnalysisSupport` | `exhaustiveness: Claim`, `cases: list[tuple[Claim, Claim]]` |
| `MathInductionSupport` | `base: Claim`, `step: Claim` |

reviewer 不直接给 cp，而是 review premise/interface claim 的 prior 和 reasoning judgment。

### 4.2 InferSupport 及其子类
...
|-------------|------------------|
| FormalSupport | judgment + justification（不直接给 cp） |
| InferSupport | 直接给 conditional probability |
| ExecutionSupport | review 前提 claims（solver 已验证等） |
| CheckSupport | review 前提 claims（测试集覆盖等） |
| FormalProofSupport | review 前提 claims + 证明系统可信度 |
...
#### ExecutionSupport / CheckSupport / FormalProofSupport
...
@dataclass
class ExtrapolationSupport(FormalSupport):
    source: Claim
    continuity: Claim

@dataclass
class EliminationSupport(FormalSupport):
    exhaustiveness: Claim
    excluded: list[tuple[Claim, Claim]]

@dataclass
class CaseAnalysisSupport(FormalSupport):
    exhaustiveness: Claim
    cases: list[tuple[Claim, Claim]]

@dataclass
class MathInductionSupport(FormalSupport):
    base: Claim
    step: Claim
...
`claim.support` 保持单值。多条 support 通过 `CompositeSupport` 聚合。
...
| `FormalSupport` | 现有 named strategy + formalization |
| `InferSupport` | `infer` / `noisy_and` |
| `ExecutionSupport` | 先在 Gaia Lang 侧定义；IR protected contract 后续设计 |
| `CheckSupport` | 同上 |
| `FormalProofSupport` | 同上 |
| `CompositeSupport` | `Strategy(sub_strategies=[...])` |

### 7.3 后续 protected-layer 工作

若 v6 在 authoring 层验证有效，再单独推进：

1. ExecutionSupport / CheckSupport / FormalProofSupport 的 protected IR contract
...
| 是否需要 Execution 作为独立概念层 | 否。吸收进 Support 子类（ExecutionSupport 等） |
| `claim.support` 是单值还是列表 | 单值。多条 support 通过 CompositeSupport 聚合 |
...
- ExecutionSupport / CheckSupport / FormalProofSupport 的 IR protected contract
