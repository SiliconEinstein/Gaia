...
`Runnable` 是所有需要"跑一个东西"的 support 的共同父类。共享字段：

- `estimated_duration`：预估运行时长
- `run_env`：运行环境描述

三个子类区分语义意图——跑的结果主张什么：

#### 4.3.1 Execution（描述性："计算得到了 X"）
...
- `execution_backend`：执行后端（python / shell / remote）

`execute()` 通常先产出一个 result claim，再通过 bridge 连接到科学结论：
...
`check()` 支撑的是 validity claim，通常不直接支撑高层 scientific claim。

#### 4.3.3 FormalProof（演绎性："定理 T 成立"）
...
最接近 Curry-Howard，但仍然可能需要 bridge claims 才能从形式模型走到科学命题。
...
#### Runnable (Execution / Check / FormalProof)
...
1. Runnable (Execution / Check / FormalProof) 的 protected IR contract
...
- Runnable (Execution / Check / FormalProof) 的 IR protected contract
