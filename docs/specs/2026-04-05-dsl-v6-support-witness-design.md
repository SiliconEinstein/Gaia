...
```
Support (base)
├── FormalSupport          # deduction, abduction, analogy, ...
├── InferSupport           # noisy_and, infer
├── ExecutionSupport       # execute()
├── CheckSupport           # check()
├── FormalProofSupport     # formal_proof()
└── CompositeSupport       # 聚合多条子 support
```

**核心原则：所有 support 构造器都是 callable 的，返回 Claim。**

```python
c = deduction("C", given=[a, b])              # → Claim (FormalSupport)
...
有 canonical skeleton 的 support，核心语义由结构决定：
...
- 内部创建 `InferSupport(family="noisy_and")`
...
| Support 用扁平 family 字段还是继承树 | 继承树。不同子类有不同的类型安全字段 |
