...
| P(E\|H), P(E\|¬H) for infer | `conditional_probabilities=[p_e_given_not_h, p_e_given_h]` when there is no `given`. With `given=G`, `premises=[H, G]` and `conditional_probabilities=[0.5, 0.5, p_e_given_not_h, p_e_given_h]`; the gate-false cells are neutral. Fixed heuristic context still goes in `background`. |
...
| `infer()` | `infer` | Existing; `premises=[H]`, `conclusion=E`, CPT = `[p_e_given_not_h, p_e_given_h]`; with `given`, append gate claims to premises and use neutral gate-false CPT cells |
