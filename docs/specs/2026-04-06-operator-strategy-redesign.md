...
Compare 是第三个 FormalStrategy 原语。它通过"度量+推断排序"比较两个预言对同一个观测的匹配度，输出一个 comparison claim。

**原子分解**：比较 = 度量（matching）+ 排序（ordering）。

- **度量**：用 equivalence 判断每个预言是否匹配观测（P ↔ Obs）
- **排序**：用 implication 从两个度量结果推断哪个更好（H_match₂ → H_match₁ 意味着"P₂ 能匹配的，P₁ 也能"）

**编译产物**：

```
equivalence([pred_h, observation], H_match1)                  # 度量：P₁ 匹配 Obs？
equivalence([pred_alt, observation], H_match2)                # 度量：P₂ 匹配 Obs？
implication([H_match2, H_match1], comparison_claim)           # 排序：P₂ 能匹配则 P₁ 也能？

warrants = [H_match1, H_match2, comparison_claim]
```

- **输入**：两个预言 claim + 一个观测 claim
- **Warrants**：3
  - H_match1："pred_h 是否匹配 observation？"（度量）
  - H_match2："pred_alt 是否匹配 observation？"（度量）
  - comparison_claim："如果 pred_alt 匹配，pred_h 也匹配吗？"（推断排序）
- **输出**：comparison_claim（implication 的 helper claim，π 由 reviewer 设定）
- **第一个预言（pred_h）默认是声称更好的那个**
- **语义**：comparison 捕捉 **"≥"（不差于）** 关系，不是严格 ">"。当两者同样匹配时 comparison 为高（"P₁ 至少和 P₂ 一样好"）

**为什么不用 deterministic operator（A∧¬B）**：比较本质上是一个**推断**（inference），不是**计算**（computation）。Reviewer 需要能对"P₁ 是否真的比 P₂ 更好"表达不确定性。Implication 提供了这个空间（reviewer 设 π），而 deterministic truth table 没有。
...
| **compare** | 2 equivalences + 1 implication | 3 | "P₁ 匹配 Obs？" + "P₂ 匹配 Obs？" + "P₂ 能匹配则 P₁ 也能？" |
...
abduction (CompositeStrategy)
  ...
  └── comparison (FormalStrategy)            ← 3 warrants (2 度量 + 1 推断排序)
  
  conclusion = comparison.conclusion (comparison claim)
```
...
def abduction(
    ...
    comparison: Strategy,       # compare(pred_h, pred_alt, obs)
    *,
    background: list[Knowledge] | None = None,
...
**作者显式提供所有三个子策略**——没有隐藏的内部创建。Compare 既可以在 abduction 内使用，也可以独立使用（A/B test）。

**Warrants 总数**：1（composition）+ 4（两个 support）+ 3（compare）= **8**

**Review checklist**（自动生成）：
1. **Composition warrant**：三个子策略是否关于同一组假说和观测？
2. **Support warrants × 4**：各理论→预言的充分性和必要性
3. **Compare warrants × 3**：两个度量（pred 匹配 obs？）+ 一个推断排序（P₁ ≥ P₂？）
...
comp = compare(pred_new, pred_old, obs_tc,
    reason="两种方法都预言 Al 在相同压力条件下的 Tc",
)

abd = abduction(s_new, s_old, comp,
    reason="三者关于同一组理论和实验",
)
# abd.conclusion = comp.conclusion (comparison claim)
```
...
| | Abduction | Induction |
|---|-----------|-----------|
| 输入 | 2 supports + 1 compare（作者显式提供） | 2 supports |
| 输出 | **comparison claim**（推断排序结果） | **law 本身**（绝对支持） |
| Composition warrant | "三个子策略关于同一组假说和观测？" | "观测独立？支持同一 law？" |
...
**Warrants**：3（H_match1 + H_match2 + comparison_claim）

**Equivalence question 模板**（度量，×2）：

> "{prediction} 是否匹配实际观测 {observation}？预言值和实验值是否一致（在合理误差范围内）？"

**Implication question 模板**（推断排序，×1）：

> "如果 {pred_alt} 能匹配观测，{pred_h} 是否也能匹配？即 {pred_h} 的预言力是否不弱于 {pred_alt}？"

**π 判定**：

| Warrant | 场景 | π |
|---------|------|---|
| H_match (equivalence) | 预言精确命中观测 | 1-ε |
| H_match (equivalence) | 预言在误差范围内 | 0.8-0.95 |
| H_match (equivalence) | 预言方向对但偏差较大 | 0.5-0.8 |
| H_match (equivalence) | 预言和观测矛盾 | ε |
| comparison (implication) | 确信 P₁ 不弱于 P₂ | 1-ε |
| comparison (implication) | 有一定信心 | 0.7-0.9 |
| comparison (implication) | 不确定 | 0.5 |
...
|---|------|----------|------------|
| 1 | abduction | 三个子策略是否关于同一组假说和观测？ | BinaryAcceptance |
...
| 8 | compare | pred_h 的匹配力 ≥ pred_alt？ | ProbabilityEstimate |

Composition warrant（#1）不通过 → 打回，比较无效。其余 7 个是 ProbabilityEstimate。
...
3. 实现 support 和 compare 作为 FormalExpr；删除 `noisy_and` 和 `soft_entailment`
4. 实现 abduction 作为 3-child CompositeStrategy（support_h + support_alt + author-provided compare）；实现 induction 作为 2-child CompositeStrategy
5. 实现 reason+prior 配对机制（DSL → formalize → lowering 全链传播）
6. 独立讨论 case_analysis / elimination 的改造
7. 更新 BP 文档（potentials.md, formal-strategy-lowering.md）

## 附录 A：Support 与 SOFT_ENTAILMENT 的关系

Support 编译为两个独立的 IMPLIES。两者与 SOFT_ENTAILMENT **趋势一致但非精确等价**。

### 行内比值一致

边缘化 helper claims 后，有效势函数的**行内比值**与 SOFT_ENTAILMENT 一致：

$$\frac{\psi(A\!=\!1, B\!=\!1)}{\psi(A\!=\!1, B\!=\!0)} = \frac{p_1}{1-p_1} \quad \text{（两者相同）}$$

$$\frac{\psi(A\!=\!0, B\!=\!0)}{\psi(A\!=\!0, B\!=\!1)} = \frac{p_2}{1-p_2} \quad \text{（两者相同）}$$

### 行间比值不同

但**行间比值**不同——两个独立 IMPLIES 引入交叉耦合：

| | Support (two IMPLIES) | SOFT_ENTAILMENT |
|---|---|---|
| ψ(1,1)/ψ(0,0) | 1 | p₁/p₂ |

当 p₁ = p₂ 时两者精确等价。当 p₁ ≠ p₂ 时 BP beliefs 有偏差（实测 |Δ| < 0.2）。
...
|------|---------|
| p₁ = p₂ → 精确匹配 | ✓ |
| p₁ 升高 → belief(B) 升高 | ✓（两模型方向一致） |
| 偏差有界 < 0.2 | ✓ |

### 设计选择

接受非精确等价。Support 是一个**新模型**，不是 SOFT_ENTAILMENT 的替代：
- 每个方向有独立可审计的 warrant
- 交叉耦合是独立 helper claim priors 的自然结果
- 趋势一致保证定性推理结论不变
