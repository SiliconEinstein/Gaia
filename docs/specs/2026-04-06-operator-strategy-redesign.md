...
> Deduction 和 biimplication 是两个基础推理原语；abduction 和 induction 是 biimplication 构建的二元 CompositeStrategy。
...
## 6. 基础推理原语：Deduction 和 Biimplication

Warrant framework 的一个直接应用是区分 Gaia 的两个基础推理原语——**deduction** 和 **biimplication**。它们覆盖了大部分常见的推理场景，也展示了 warrant 在单向和双向推理中的不同角色。
...
### 6.2 Biimplication — 双向推理

Biimplication 是双向推理原语：两个命题之间存在两个独立的 implication，各自有自己的 warrant。
...
### 6.3 Biimplication = FormalExpr，不是新算符

Biimplication 不是一个新的 4 元 operator——它是 **FormalExpr 层面的组合模式**，编译为两个 ternary IMPLIES operator。这个设计选择有三个理由：
...
### 6.4 Biimplication 吸收 noisy_and 和 soft_entailment

原来独立的 `noisy_and` 和 `soft_entailment` 策略类型都是 biimplication 的参数特例：
...
所有这些场景使用**同一个 biimplication FormalExpr 结构**，只是 $p_1$ 和 $p_2$ 的取值不同。**`noisy_and` 和 `soft_entailment` 作为独立策略类型被删除**——它们吸收进 biimplication。

当 $p_2 = 0.5$ 时，反向 implication 对 BP 没有额外信息贡献，H_rev 实际上是 silent 的——此时 biimplication 在数学上退化为单向的 deduction。作者选择 biimplication 而非 deduction 的差别在于**想不想显式暴露反向 warrant 作为审计槽位**，即使它暂时是 silent。

### 6.5 Deduction vs Biimplication 的选择

作者在这两个原语之间的选择取决于**他想向 reviewer 声明什么**：

| | Deduction | Biimplication |
|---|-----------|-----------|
...
### 6.6 Biimplication 作为 Warrant framework 的印证

Biimplication 是一个很好的例子，展示本 spec 的 warrant framework 的实际工作方式：

- **每个 relation operator 产生一个 warrant**：biimplication 有两个 implication，所以有两个 warrant
...
Abduction 和 induction 不是独立的 FormalStrategy 原语——它们是**由 biimplication 构建的二元 CompositeStrategy**。

### 7.1 共同结构

两者都是二元 CompositeStrategy，接受**恰好 2 个 biimplication** 作为子策略：

```
CompositeStrategy(type=abduction|induction):
  sub_strategies = [biimplication_1, biimplication_2]
  conclusion = <见下文>
```
...
```
biimplication(H1, Obs)    # 2 warrants (H1→Obs 预言力, Obs→H1 反向支持)
biimplication(H2, Obs)    # 2 warrants (H2→Obs 预言力, Obs→H2 反向支持)
...
- **Posterior 由 BP 从 4 个 biimplication warrants 推出**，不由 reviewer 直接设
- 行为类似 computation operator 的 helper claim——值由结构决定

**Warrants 数量**：4（2 per biimplication）。H_comp 贡献 0 个 warrant。
...
```
biimplication(H, Obs1)    # 2 warrants
biimplication(H, Obs2)    # 2 warrants
...
每一步 reviewer 审 2 个新 warrants（新增 biimplication 的 forward + reverse）。

### 7.4 Abduction vs Induction 的关键不对称

| | Abduction | Induction |
|---|-----------|-----------|
| 输入 | 2 biimplications（2 个假说，1 个 Obs） | 2 biimplications（1 个假说，2 个 Obs） |
...
biimplication(shared_bias, Obs1)
biimplication(shared_bias, Obs2)
```

这些约束有**自己的 warrants**（contradiction 和 equivalence/biimplication 的 helper claims），独立于 abduction/induction 本身的 warrants。Reviewer 分别审计。
...
- 4 个 biimplication warrants（每个假说的预言力 + 反向支持）
- 如果有 contradiction/disjunction：假说间互斥/穷尽的 warrants

**Induction 的 reviewer 关注**：
- 每步新增 2 个 biimplication warrants（新观测的预言力 + 反向支持）
...
| — | biimplication (FormalStrategy) | 基础推理原语，被以上三者使用 | — |

### 8.3 Jaynes 弱三段论的实现

每个 biimplication 在因子图中实现 Jaynes 的弱三段论：

- Forward implication 编码 $P(Obs|H)$（假说对观测的预言力）
- Reverse implication 编码 $P(H|Obs)$ 的反向推理强度
- BP 按 Bayes 公式计算后验

Abduction 通过比较两个 biimplication 的 warrants 来判断"哪个假说更好"——本质上比较 $P(Obs|H_1)$ vs $P(Obs|H_2)$。

### 8.4 Peirce 与 Jaynes 的统一

Gaia 同时实现了 Peirce 和 Jaynes 的框架：

- **Jaynes 的数学**：每个 biimplication 的 warrants 编码 Baynes 弱三段论的参数，BP 严格按 Bayes 公式计算
...
2. **两个推理原语**（§6）：deduction（单向，1 warrant）和 biimplication（双向，2 warrants）
...
3. 实现 biimplication 作为 FormalExpr；删除 `noisy_and` 和 `soft_entailment`
...
当前 IR 文档（02-gaia-ir.md）的 abduction 使用 `disjunction + equivalence` 结构。本 spec 提议的 "biimplication-based CompositeStrategy" 是一种**替代设计**。两者在简单情况下数学等价（Jaynes 弱三段论），但新设计：

- 提供 per-hypothesis 的独立审计（每个 biimplication 有自己的 warrants）
- 支持 N>2 假说的链式比较
- 将穷尽性/互斥约束分离为独立的图结构
...
| **Comparison helper claim** | Abduction 自动生成的输出 claim（π=0.5，无 warrant，posterior 由 BP 从 biimplication warrants 推出） |
| **Forward warrant** | 高 π 加强推理的 warrant（implication H） |
| **Reverse warrant** | 作用方向相反：高 π 反向支持某假说（biimplication 的 reverse implication H） |
| **Reasoning primitive** | Deduction（1 IMPLIES）或 Biimplication（2 IMPLIES）——所有复合策略的构建块 |
| **CompositeStrategy** | 二元组合：abduction（比较）或 induction（累积），由 biimplication 构建 |
| **Structural constraint** | 独立于策略的 graph operator（contradiction, disjunction, complement），作者按需添加。观测间共享因素通过普通 claim + biimplication 建模 |
...
策略管推理（hypothesis ↔ observation），图结构管约束（hypothesis ↔ hypothesis）。观测间的共享因素（如系统偏差）通过普通的 claim + biimplication 建模，不需要新 operator。两者正交，BP 联合推理。
