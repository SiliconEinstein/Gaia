...
### 6.4 Support 吸收 noisy_and 和 soft_entailment

原来独立的 `noisy_and` 和 `soft_entailment` 策略类型都是 support 的参数特例：
...
2. **两个推理原语**（§6）：deduction（单向，1 warrant）和 support（双向，2 warrants）
