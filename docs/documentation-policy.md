...
| 关联文档 | [文档导航首页](README.md), [01-product-scope.md](foundations/ecosystem/01-product-scope.md) |
...
## 16. How-To Guides 贡献规范

How-To Guides 是面向任务的短配方——读者知道自己想做什么，只是不知道具体怎么操作。

### 16.1 什么适合写成 How-To

- 读者有一个**具体任务**（不是"我想了解 Gaia"）
- 解决方案可以在**一篇文档内完成**
- 假设读者已完成 Quick Start，具备基本概念
...
**Gaia version 是必填字段。** 文档中的代码和 CLI 命令必须在该版本下验证通过。当 Gaia 发布 breaking change 时，维护者应标记受影响的 How-To 为过时或更新。

### 16.3 审查标准

| # | 标准 | 不通过的典型情况 |
|---|------|----------------|
| 1 | **任务聚焦** — 解决一个具体问题，标题是 "How to …" | 标题是概念名或功能名而非任务 |
| 2 | **可操作** — 读者跟着步骤走能得到结果 | 步骤含糊、跳跃、依赖未说明的前置条件 |
| 3 | **不重教基础** — 假设 Tutorial 知识，不重新解释基本概念 | 大段介绍 claim/derive/observe 是什么 |
| 4 | **深度链接化** — "为什么"链接到 Foundations | 内联解释理论背景 |

### 16.4 与相邻区的边界

- 如果写着写着变成了从零教学 → 它是 Tutorial
- 如果写着写着变成了一个完整领域故事 → 它是 Example
- 如果写着写着变成了 API 签名列举 → 它是 Reference

## 17. Examples 贡献规范

`docs/examples/` 下的每篇文档是一个完整的领域场景，展示 Gaia 在真实问题中的用法。

### 17.1 与 How-To 的区别

- **How-To** 是短配方——读者有一个具体任务，文档给出最短路径
- **Example** 是完整案例——读者想看一个领域场景如何端到端地用 Gaia 表达
...
[领域背景。让领域专家认出这是他们的问题。
不出现 Gaia 术语——用领域自身的语言描述问题。]

## 代码

[完整可运行的 Gaia package 代码。
行内注释聚焦"为什么这样写"，不解释 Gaia 基础概念。]

## 运行

[CLI 命令 + 预期输出。读者可以直接复制运行：
gaia build compile + gaia run infer]

## 关键机制

[只写本例中有独立价值的点。每个机制：
 - 做了什么、为什么这样做（2-3 句）
 - → 详见 [链接到 Foundations 或 Reference 的具体页面]

不重复已有文档的内容。]
```

**Gaia version 是必填字段。** 代码必须在该版本下 `gaia build compile` + `gaia run infer` 通过。当 Gaia 发布 breaking change 时，维护者应标记受影响的 Example 为过时或更新。

### 17.3 审查标准

| # | 标准 | 不通过的典型情况 |
|---|------|----------------|
| 1 | **场景真实** — 来自具体领域，不是玩具 | 用 `foo`/`bar` 做示例变量 |
| 2 | **有独立价值** — 展示 Foundations/Reference 中缺少实战说明的机制或组合 | 所有用到的机制已在其他文档有充分实战说明 |
| 3 | **解释链接化** — 引用而非重复已有文档 | 大段重新解释 BP 联合更新、Cromwell 钳制等已有 Foundations 内容 |
| 4 | **可运行** — `gaia build compile` + `gaia run infer` 通过 | 代码片段无法独立编译 |

### 17.4 Reviewer 行动

- 如果一篇 Example 的"关键机制"段落与 Foundations/Reference 内容重复，Reviewer 应要求作者删除重复段落并替换为链接
- 如果场景为玩具或缺乏领域背景，Reviewer 应要求补充真实场景描述
- 如果无法证明增量价值，Reviewer 可建议将内容合并到已有 Example 或降级为代码注释
