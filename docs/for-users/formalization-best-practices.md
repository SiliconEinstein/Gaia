...
│   └── Distribution + bayes.model + observe(dist, value, error) + bayes.likelihood
│        （详见 §3）
│
└── 作者主观似然度判断
...
错在哪：distribution 函数会从 `(mu, sd)` 自动算出每个假设下 observed value 的似然度，作者不需要也不应该手工给 `p_e_given_h`。这是把应该走 (B) 客观似然度的场景错写成 (C) 主观。
...
**distribution 的参数和 `observe(dist, value, error)` 的数据必须来自不同来源。**
...
写完 `bayes.model(..., distribution=Normal(mu=X, sd=Y))` 后问自己：**"`X` 和 `Y` 这两个数字，我是怎么知道的？"** 如果答案是"从我马上要 `observe()` 进 likelihood 的同一份数据里算出来的"，回去 §2 选 `infer` 而不是 `bayes`，或者找一份独立的 prior 数据集。
...
- [ ] `bayes.model` 的参数和 `observe` 的数据不是同一份——CLI 不知道你的数据来源
