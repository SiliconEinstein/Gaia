...
def test_predicate_claim_distribution_vs_distribution_not_supported():
    """Threshold being another Distribution would require joint marginalisation."""

    def make() -> None:
        A = Normal("A", mu=10, sigma=1)
        B = Normal("B", mu=5, sigma=1)
        bad = claim("A bigger than B", A > B)
        bad.label = "bad"

    with pytest.raises(NotImplementedError, match="Distribution on both sides"):
        _compile_with(make)


def test_predicate_claim_ne_operator_routes_to_equation_slot():
    """The ``!=`` op is treated as an equation proposition, like ``==``."""
    A = Normal("A", mu=10, sigma=1)
    B = Normal("B", mu=10, sigma=1)
    ne_claim = claim("A unequal to B", A != B, prior=0.4)
    assert "equation" in ne_claim.metadata
    assert "predicate" not in ne_claim.metadata
    assert ne_claim.metadata["equation"].op == "!="


def test_distribution_metadata_is_always_dict_even_when_unitless():
    """Regression: unitless Distribution must have ``metadata == {}`` not ``None``.

    Earlier snapshots returned ``None`` from ``_attach_units`` on the
    unitless path, breaking the Knowledge invariant that ``metadata`` is
    dict-typed and forcing defensive ``(meta or {})`` patterns everywhere.
    """
    n = Normal("plain", mu=0, sigma=1)
    assert isinstance(n.metadata, dict)
    assert n.metadata == {}
    # The .get() call must not raise — it would on a ``None`` metadata.
    assert n.metadata.get("unit") is None
