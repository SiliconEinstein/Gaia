...
def test_lang_lazy_bayes_attribute_points_to_peer_module() -> None:
    _drop_bayes_modules()
...
def test_lang_bayes_namespace_is_tombstoned() -> None:
    _drop_bayes_modules()

    with pytest.raises(ImportError, match=r"moved to gaia\.engine\.bayes"):
        from gaia.engine.lang.bayes import model  # noqa: F401


def test_lang_bayes_submodule_import_is_tombstoned() -> None:
    _drop_bayes_modules()

    with pytest.raises(ImportError, match=r"moved to gaia\.engine\.bayes\.compiler"):
