...
def test_focus_multiple_flags_rejected(tmp_path):
    ...
    _simple_pkg(pkg)
    r = runner.invoke(
        app, ["inquiry", "focus", "--push", "--pop", "--path", str(pkg)]
    )
    assert r.exit_code == 2
...
def test_obligation_add_and_list(tmp_path):
    ...
    r2 = runner.invoke(
        app, ["inquiry", "obligation", "list", "--json", "--path", str(pkg)]
    )
    assert r2.exit_code == 0
...
def test_obligation_close(tmp_path):
    ...
    _simple_pkg(pkg)
    r = runner.invoke(
        app, ["inquiry", "obligation", "add", "X", "-c", "a",
              "--kind", "prior_hole", "--path", str(pkg)],
    )
    assert r.exit_code == 0
    qid = r.output.strip().split()[2]

    r2 = runner.invoke(
        app, ["inquiry", "obligation", "close", qid, "--path", str(pkg)]
    )
    assert r2.exit_code == 0
    assert "closed" in r2.output

    r3 = runner.invoke(
        app, ["inquiry", "obligation", "list", "--path", str(pkg)]
    )
...
def test_obligation_close_nonexistent_fails(tmp_path):
    ...
    _simple_pkg(pkg)
    r = runner.invoke(
        app, ["inquiry", "obligation", "close", "no_such", "--path", str(pkg)]
    )
...
def test_hypothesis_add_list_remove(tmp_path):
    ...
    _simple_pkg(pkg)
    r = runner.invoke(
        app, ["inquiry", "hypothesis", "add", "训练数据 i.i.d.",
              "--scope", "github:proof_pkg::main_claim", "--path", str(pkg)],
    )
    assert r.exit_code == 0
    qid = r.output.strip().split()[2]

    r2 = runner.invoke(
        app, ["inquiry", "hypothesis", "list", "--json", "--path", str(pkg)]
    )
    rows = json.loads(r2.output)
    assert len(rows) == 1
    ...
    r3 = runner.invoke(
        app, ["inquiry", "hypothesis", "remove", qid, "--path", str(pkg)]
    )
    assert r3.exit_code == 0
...
def test_reject_strategy(tmp_path):
    ...
    _simple_pkg(pkg)
    r = runner.invoke(
        app, ["inquiry", "reject", "lcs_bad_s3", "-c", "loop", "--path", str(pkg)]
    )
    assert r.exit_code == 0
...
def test_tactics_log_captures_events(tmp_path):
    ...
    runner.invoke(
        app, ["inquiry", "obligation", "add", "X", "-c", "a",
              "--kind", "other", "--path", str(pkg)],
    )
    r = runner.invoke(
        app, ["inquiry", "tactics", "log", "--json", "--path", str(pkg)]
    )
    assert r.exit_code == 0
...
def test_review_renders_proof_state_section(tmp_path):
    ...
    _simple_pkg(pkg)
    runner.invoke(
        app, ["inquiry", "obligation", "add", "github:proof_pkg::main_claim",
              "-c", "need prior", "--kind", "prior_hole", "--path", str(pkg)],
    )
...
def test_review_json_includes_proof_context(tmp_path):
    ...
    _simple_pkg(pkg)
    runner.invoke(
        app, ["inquiry", "hypothesis", "add", "iid", "--path", str(pkg)],
    )
