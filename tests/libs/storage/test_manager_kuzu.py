...
async def test_manager_kuzu_graph_operations(tmp_path):
    """StorageManager with Kuzu backend should support full CRUD."""
    config = StorageConfig(
        lancedb_path=str(tmp_path / "lance"),
        graph_backend="kuzu",
    )
    ...
    edge = HyperEdge(id=1, type="deduction", tail=[10], head=[20], probability=0.8)
