...
The first binding name matches the hand-authored binding name verbatim. The
...
gaia author derive \
  ...
  --dsl-binding-name aristotle_daily_observation_path \
  --target ./galileo-cli-mirror-gaia
...
gaia author equal \
  ...
  --dsl-binding-name aristotle_daily_match \
  --target ./galileo-cli-mirror-gaia
...
gaia author equal \
  ...
  --dsl-binding-name medium_daily_match \
  --target ./galileo-cli-mirror-gaia
```
...
gaia author derive \
  ...
  --dsl-binding-name aristotle_composite_faster \
  --target ./galileo-cli-mirror-gaia
...
gaia author derive \
  ...
  --dsl-binding-name aristotle_composite_slower \
  --target ./galileo-cli-mirror-gaia
...
gaia author contradict \
  ...
  --dsl-binding-name aristotle_paradox \
  --target ./galileo-cli-mirror-gaia
```
...
gaia author derive \
  ...
  --dsl-binding-name medium_vacuum_equal_fall_prediction \
  --target ./galileo-cli-mirror-gaia
```
...
hand-authored package on every axis other than how each statement's IR
label is spelled at the source level, which is intrinsic-by-design.

### Binding name carries the IR label (intrinsic)

The hand-authored file sometimes uses different identifiers for the
...
The walkthrough passes only
`--dsl-binding-name aristotle_daily_observation_path` (and no
`--label`), so the cli renders a bare
`aristotle_daily_observation_path = derive(...)` with no `label=`
kwarg; the engine's package loader then defaults the compiled IR label
to the binding name. The hand-authored form reaches the same IR label
through an explicit `label=` kwarg on a distinct binding. Subsequent
...
`derive` / `equal` / `contradict` also expose a separate optional
`--label <engine-label>` that renders an explicit engine `label=`
kwarg when a binding and its IR label must differ; the walkthrough
never needs it, because naming each binding after its intended label
is the simplest shape that reproduces the hand-authored IR.

The galileo equivalence test asserts the **label-bag** axis on this
dimension at CONTENT_SET, rather than label identity — both sides
produce the same set of referenceable labels.
...
The only remaining source-text divergence is that each cli statement
carries its IR label via the `--dsl-binding-name` binding name, which
is non-semantic at the IR level.
