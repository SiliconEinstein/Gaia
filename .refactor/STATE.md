...
- [x] **2.5.3c** Refactor 13-29 complexity band C901 (~46 functions across modules). Executor groups by module: 2.5.3c-cli-commands / 2.5.3c-ir / 2.5.3c-bp / 2.5.3c-inquiry / 2.5.3c-lang / 2.5.3c-trace / 2.5.3c-logic. Module-internal C901 functions handled together within each unit.
  - status: `done`
  - breakpoint_notes: Split live 45 remaining C901 findings into module sub-units on 2026-05-12: bp (8), cli+cli/commands (20), inquiry (2), ir (8), lang (7). 2.5.3c-bp, 2.5.3c-cli, 2.5.3c-inquiry, 2.5.3c-lang, and 2.5.3c-ir are done. No live trace/logic findings remained. Full-select ruff now has 0 findings; Phase 2.5.4 close-out acceptance remains pending as the next work unit.
...
- [x] **2.5.3c-cli** Refactor CLI 13-29 complexity C901 functions across `gaia/cli/` and `gaia/cli/commands/`. Preserve CLI command names, arguments, output formats, persisted artifact shapes, replay/starmap behavior, and render/check/register semantics.
  - status: `done` | claimed_by: Cursor GPT-5.5 | claimed_at: 2026-05-12 17:08 | completed_at: 2026-05-12 17:42 | breakpoint_notes: Refactored the live 20 CLI C901 findings with private helper extraction across package loading/manifests/fills bridge validation, brief/detailed/GitHub/wiki/Obsidian/graph JSON/Mermaid renderers, quality gate checks, replay round-belief computation, and infer/render/starmap/trace command validation paths. Public Typer command signatures, command names, output formats, persisted artifact paths/shapes, replay/starmap behavior, register/check/render/infer semantics, and review/trace surfaces are preserved. Verification: `uv run ruff check gaia/cli --select C901 --output-format=concise --exit-zero` => passed; `uv run ruff check gaia/cli --output-format=concise && uv run ruff format --check gaia/cli && uv run mypy gaia/cli --show-error-codes --no-pretty` => passed; `uv run pytest tests/cli --no-cov` => 414 passed, 3 skipped; final `uv run ruff check . --statistics --exit-zero && uv run ruff format --check . && uv run mypy --show-error-codes --no-pretty && uv run pytest && uv run pre-commit run --all-files` => 17 remaining findings, all C901 outside CLI reserved for later 2.5.3c sub-units; 280 files formatted; mypy success for 275 source files; pytest 1605 passed, 3 skipped, 58 warnings, TOTAL coverage 91.48%, required 90% reached; pre-commit hooks passed.
...
- [x] **2.5.3c-ir** Refactor IR 13-29 complexity C901 functions in `gaia/ir/`. Preserve IR schemas, validators, hashing, public symbols, and protected-layer semantics.
  - status: `done` (was `blocked` 2026-05-12 17:52 → unblocked 2026-05-12 18:35 after M3 resolved per user option B) | claimed_by: Cursor GPT-5.5 | claimed_at: 2026-05-12 18:36 | completed_at: 2026-05-12 18:46
  - breakpoint_notes: M3 doc-code contradiction (`OperatorType.IMPLICATION` arity) resolved by user-authorized doc edit on 2026-05-12: `docs/foundations/gaia-ir/02-gaia-ir.md` §2.2 table line 193 updated from `[A] | B` to `[A, B] | helper claim（如 implies(A,B)）`; `docs/foundations/gaia-ir/08-validation.md` §3 item 6 tightened to describe `implication`'s conclusion as the structural implication helper claim with `variables=[A, B]`. Live 2-variable + helper-claim model is now canonical. Refactored the 8 remaining IR C901 functions (`Operator._validate_invariants`, `Strategy._compute_id_and_validate`, and validator knowledge/strategy/FormalExpr/private-node/scope/compose validation helpers) into private helper phases without changing IR schemas, Pydantic field names, validation messages, public symbols, hashing, or protected-layer semantics. Verification: `uv run ruff check gaia/ir --select C901 --output-format=concise` => passed; `uv run ruff check gaia/ir --output-format=concise` => passed; `uv run mypy gaia/ir --show-error-codes --no-pretty` => success for 13 source files; `uv run pytest tests/ir tests/gaia/ir --no-cov` => 245 passed; `uv run ruff check . --statistics --exit-zero` => 0 findings; full Phase 2.5 gate `uv run ruff check . && uv run ruff format --check . && uv run mypy && uv run pytest --cov` => ruff passed, 280 files formatted, mypy success for 275 source files, pytest 1605 passed / 3 skipped / 58 warnings, TOTAL coverage 95.14%, required 90% reached.
...
### M3 — `OperatorType.IMPLICATION` arity mismatch between protected IR docs and live code/tests ✅ FIXED

- **Found at**: Phase 2.5.3c-ir on 2026-05-12 17:52 by cursor-agent (Round 15)
- **Doc location (before fix)**: `docs/foundations/gaia-ir/02-gaia-ir.md` §2.2 table described `implication` as `variables=[A]`, `conclusion=B`; `docs/foundations/gaia-ir/08-validation.md` §3 item 6 described `implication`'s conclusion as "输出 claim".
- **Actual code location**: `gaia/ir/operator.py:_validate_invariants` requires `operator=implication` to have exactly 2 variables; `gaia/bp/lowering.py:_lower_deduction_implication` reads `antecedent = op.variables[0]`, `consequent = op.variables[1]`, `op.conclusion = helper claim` popped from factor graph; IR/BP tests construct implication operators with two variables plus a distinct conclusion.
- **Nature**: semantic contract mismatch in the IR operator schema/validation surface. Not a docstring or typing issue; live behavior was "2 input variables + helper claim conclusion", doc table was stale "1 input variable + output claim conclusion".
- **Impact area**: IR operator validation, FormalExpr closure rules, formalization helpers, BP lowering, and tests.
- **Resolution**: User chose option B (live code canonical) on 2026-05-12 ~18:30 and explicitly authorized the doc-side fix to the protected layer. Orchestrator surgically edited two doc files in the same M3-resolution commit:
  1. `docs/foundations/gaia-ir/02-gaia-ir.md` §2.2 table line 193: `[A] | B` → `[A, B] | helper claim（如 implies(A,B)）`. Aligns implication with the other relation operators (equivalence / contradiction / complement / disjunction) which already use `[A, B] + helper claim`.
  2. `docs/foundations/gaia-ir/08-validation.md` §3 item 6: tightened `Directed（implication、conjunction）` wording — `implication`'s conclusion is the structural implication helper claim (with `variables=[A, B]`), distinct from `conjunction`'s conclusion which is the compositional output claim `M`.
- **§2.4 in 02-gaia-ir.md**: not changed. Pre-existing wording "输出 claim 或 implication helper（取决于 formalization 模板）" already accommodated the helper-claim interpretation via the "或" disjunction — left as-is to minimize blast radius.
- **Status**: ✅ FIXED 2026-05-12 ~18:35 in the same orchestrator commit batch (docs + STATE.md). 2.5.3c-ir unblocked and back in queue as `pending`.
- **Mirror source**: `home_agent/projects/gaia/refactor-batch-review.md` § "2026-05-12 — M3 doc-code contradiction" (orchestrator-side pre-resolution ledger entry).
