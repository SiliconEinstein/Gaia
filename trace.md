...
Patching causal spec on docs/causal-design-revisions branch. Two big design decisions baked in this round: (1) **modality-aware mutilation** — `do()` only severs causal SOFT_ENTAILMENT factors (those tagged `metadata.causal.dag_edge`); deduction-derived factors and EQUIVALENCE/CONJUNCTION/DISJUNCTION operator factors are preserved. Justified by 8 thought experiments showing this is forced by the meaning of `do()` (interventions act on the world, not on logical statements). The earlier "definitional flag" escape hatch is dropped — redundant under first principles. (2) **per-instance grounding** for `forall(p, Causes(X(p), Y(p)))` — strict dual to v0.5's existing logical-quantifier grounding (`gaia/lang/compiler/lower_formula.py:107-192`): emit one universal claim + per-domain-member instance Knowledge nodes + deduction edges from universal to instances + one cause()-derived SOFT_ENTAILMENT per instance pair. Population-level intervention and CATE explicitly deferred to v0.7+. Also: `cause()` is the new fourth-family verb (alongside Relate/Correlate/Strategy per foundation spec §18); `causes()` lowercase formula constructor removed in favor of `Causes()` direct dataclass; multi-parent effects compose via noisy-OR by default; ATE wrapper added; soft-do (`do(X~Bernoulli)`) split out to D₂.5.
