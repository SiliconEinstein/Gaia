...
### EARS — Progress (2026-05-05 20:03)
<!-- concepts: causal-reasoning, action-hierarchy, role-on-action-graph -->
Patching causal spec (`docs/specs/2026-05-05-causal-reasoning-design.md`) for v0.5's PR #524 changes (role-on-action-graph + decompose). Two important corrections to my earlier mental model: (1) `causal()` already shipped on v0.5 in `gaia/lang/dsl/sugar.py:86` as a **claim factory** (returns Claim, not Action) — it's NOT an action verb. So the right shape for v0.6 is to add a NEW `Cause` Action subclass under the `Probabilistic` family (parallel to `Infer` and `Associate`) carrying CPD params, plus a NEW `cause()` DSL verb that creates both a `causal()` claim AND registers the `Cause` action. This mirrors how `infer()` returns a Claim while the `Infer` action carries `p_e_given_h`. (2) The "fourth verb family alongside Relate / Correlate / Strategy" framing is gone — Relate is demoted to private, public hierarchy is now `Action / Support / Structural / Probabilistic / Scaffold / Compose` per `gaia/lang/runtime/action.py`. Causal lives **under Probabilistic**, not as a sibling family.
