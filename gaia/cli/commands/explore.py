...
def _load_manifest(pkg: str) -> dict[str, Any] | None:
    """Load ``.gaia/formalization_manifest.json`` if present (for depends_on)."""
    p = _gaia_dir(pkg) / "formalization_manifest.json"
    if not p.exists():
        return None
    try:
...
def frontier_command(
    ...
    Loads the map, compiles the IR graph (reusing the inquiry graph loader),
    folds in the formalization manifest's ``depends_on`` scaffolds and the
    flattened beliefs, then runs ``extract_frontier`` → ``reconcile_frontier`` →
    ``score_frontier`` and saves. Prints the ranked top-k open contacts (k =
    ...
    extracted = extract_frontier(graph, exploration_map, formalization_manifest=manifest)
    reconcile_frontier(exploration_map, extracted, discovered_round=exploration_map.round)
    score_frontier(exploration_map, beliefs=beliefs, ir=graph)
    _refresh_stats(exploration_map)
    save_map(pkg, exploration_map)
...
def round_command(
    ...
    typer.echo(
        ...
        f"{len(surveyed or [])} surveyed, {open_after} open contact(s)."
    )
