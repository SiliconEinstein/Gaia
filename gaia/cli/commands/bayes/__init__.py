...
* ``bayes likelihood``— compare observed data against one or more
                        predictive models.
...
and ``likelihood`` produce helper Claim bindings the same way the
engine's ``bayes.model`` / ``bayes.likelihood`` do.
"""
...
from gaia.cli.commands.bayes.likelihood import likelihood_command
