...
def normalize_lkm_reasoning_search(payload: dict[str, Any], *, query: str) -> dict[str, Any]:
    ...
    return _envelope(query=query, provider="lkm", kind="derive", results=results)
    ...
    *,
    ...
    if object_kind == "claim" and provider_id and variable.get("has_reasoning") is not False:
        actions.append(
            {
                "kind": "inspect",
                "command": f"gaia search lkm reasoning {provider_id}",
            }
        )
...
def _normalize_lkm_chain(
    ...
    *,
    ...
    provider_id = _string(chain.get("id")) or f"chain_{index}"
    ...
    return {
        ...
        "gaia": _gaia_identity("derive"),
        "source": {
            "provider_id": provider_id,
            "source_package": source_package,
            "paper_id": paper_id,
            "doi": _string(paper.get("doi")),
            "conclusion_id": _string(conclusion.get("id")),
        },
...
def _gaia_identity(object_kind: str) -> dict[str, str | None]:
    return {
        "qid": None,
        "label": None,
        "package": None,
...
def _chain_conclusion(chain: dict[str, Any]) -> dict[str, Any]:
    ...
    for factor in factors:
        ...
        if conclusion:
            return conclusion
    return _dict(chain.get("conclusion"))
