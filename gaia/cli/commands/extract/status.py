...
_STATUS_EPILOG = (
    ...
    "  succeeded          ->  result\n\n"
    "  partial or failed  ->  result; do not resubmit the same PDF "
    "or `--content` body on partial\n\n"
    "`status` reads the task once and exits; it does not block. Use "
    "`gaia extract submit --wait` when you want the CLI to poll for you.\n\n"
    "`stage` is progress narration (examples include metadata, ocr, "
    "step0-step4, step2_3, graph, done); the list is not exhaustive. "
