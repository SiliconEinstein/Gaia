...
_SUBMIT_EPILOG = (
    ...
    "the same PDF or `--content` body reuses the existing extraction "
    "rather than starting over, so an already-processed paper comes back "
    "terminal immediately with `cache_hit`. `cache_source` says where that "
    "reuse came from: `lkm` when the paper was already extracted in the "
    "corpus, `local` when an earlier submission of this same PDF or "
    "`--content` body produced it. Resubmitting will not hurry a running "
    "task along: the same user sending the same PDF or `--content` body "
    "while it is still queued or running returns business error 290020 with "
