...
def _collect_dependency_action_roles(action: Action, add: RoleAdder) -> bool:
    """Collect roles for dependency and candidate-relation actions."""
...
def _collect_infer_action_roles(action: Action, add: RoleAdder) -> bool:
    """Collect roles for probabilistic infer and associate actions."""
    if isinstance(action, Infer):
...
def _collect_relation_action_roles(action: Action, add: RoleAdder) -> bool:
    """Collect roles for pairwise relation operator actions."""
...
def _collect_structural_action_roles(
    action: Action,
    add: RoleAdder,
    occurrences: list[RoleOccurrence],
    *,
    ...
    """Collect roles for decomposition, composition, and support actions."""
...
def _collect_builtin_action_roles(
    action: Action,
    add: RoleAdder,
    occurrences: list[RoleOccurrence],
    *,
    ...
    """Collect roles for built-in Gaia Lang action classes."""
    if _collect_observation_action_roles(action, add):
        return
    if _collect_dependency_action_roles(action, add):
        return
    if _collect_infer_action_roles(action, add):
        return
    if _collect_relation_action_roles(action, add):
        return
    _collect_structural_action_roles(
        action,
        add,
        occurrences,
        path=path,
...
def _collect_common_action_roles(
    action: Action,
    add: Callable[..., None],
    *,
    include_background: bool,
    ...
    """Collect background and warrant roles shared by every action."""
