"""ReviewManifest loading + merge helpers (engine-side facade entry).

`load_or_generate_review_manifest` is publicly re-exported via
`gaia.engine.inquiry.__all__`. The other helpers (merge / latest_reviews /
REVIEW_MANIFEST_REL_PATH) are engine-internal helpers used by CLI commands.
"""
...
from gaia.engine.ir import Review, ReviewManifest
...
def latest_reviews(manifest: ReviewManifest) -> list[Review]:
    """Return the most recent review per target, ordered by action label."""
