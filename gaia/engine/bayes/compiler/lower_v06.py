"""v0.6 Bayes lowering — Prediction / ModelComparison → IR.

Counterpart to the v0.5 :mod:`gaia.engine.bayes.compiler.lower`.
Dispatches on :class:`Prediction` and :class:`ModelComparison` (the v0.6
Actions produced by :func:`predict` and :func:`compare`), and reads the
unified ``metadata["observation"]`` / ``metadata["prediction"]`` schema
written by the v0.6 DSL surface.
...
* ``ModelComparison`` actions emit one ``infer`` strategy per hypothesis
  whose CPT is ``[0.5, clamp(LR_i)]`` with ``LR_i = exp(logL_i -
  logL_max)``. This matches the v0.5 lowering exactly so v0.5 numeric
  outputs are reproduced.
* Exclusivity contracts ("pairwise_contradiction" /
  "exhaustive_pairwise_complement" / "none") emit Structural Actions and
  the clamped Disjunction helper operator for ≥3 hypotheses, identical
  to v0.5.

What this module does NOT own:

* Reading observation value or noise out of ``claim.formula`` — that is
  the v0.5 storage path. v0.6 reads only ``metadata["observation"]``.
* The legacy ``metadata["bayes"]`` schema written by ``bayes.model`` /
  ``bayes.likelihood`` — handled by :mod:`gaia.engine.bayes.compiler.lower`.
"""
...
class BayesV06LoweringResult:
    """IR additions and metadata updates emitted by v0.6 Bayes lowering."""
...
def lower_v06_bayes_claims(
    ...
    *,
    ...
    """Lower v0.6 ``Prediction`` and ``ModelComparison`` actions to IR."""
    del knowledge_nodes
...
def _prediction_metadata(
    ...
    *,
    ...
    metadata: dict[str, Any] = {"prediction": payload, "bayes": {"role": "prediction"}}
    if action_label:
...
def _bind_distribution_impl(distribution: Distribution, hypothesis: Claim) -> Any:
    ...
    Mirrors the v0.5 ``_bind_distribution`` helper but operates on the
    Distribution Knowledge wrapper. Returns the underlying pydantic
    ``_BaseDistribution`` with concrete params, ready for logpdf/logpmf.
    """
...
def _claim_parameter_bindings(claim: Claim) -> _Bindings:
    ...
    Reuses the v0.5 formula-pair extractor: hypothesis formulas are
    expected to be ``Equals(variable, Constant)`` or conjunctions thereof.
    """
    from gaia.engine.bayes.compiler.lower import _equals_variable_constant_pairs
...
def _observation_value(data_claim: Claim, target: Variable | Distribution) -> Any:
    ...
    Unlike the v0.5 lowering which walked ``claim.formula``, this reader
    operates only on the v0.6 metadata schema. The match between
    observation and prediction targets is by Python object identity.
    """
...
def _log_likelihood_with_noise(
    ...
    """Convolve predictive with Normal additive noise. Mirrors v0.5 algorithm."""
    if noise.kind != "normal":
        raise NotImplementedError(
            "v0.6 Bayes compare currently supports only Normal additive noise"
        )
